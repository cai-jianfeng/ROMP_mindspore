...
def padding_image(image):
    ...
    image_pad_info = mindspore.Tensor([top, bottom, left, right, h, w])
...
def img_preprocess(image, input_size=512):
    ...
    input_image = mindspore.Tensor.from_numpy(cv2.resize(pad_image, (input_size,input_size), interpolation=cv2.INTER_CUBIC))[None].float()
...
def convert_tensor2numpy(outputs, del_keys=['verts_camed','smpl_face', 'pj2d', 'verts_camed_org']):
    ...
    for key in result_keys:
        if isinstance(outputs[key], mindspore.Tensor):
...
def smooth_global_rot_matrix(pred_rots, OE_filter):
    ...
    smoothed_rot = mindspore.Tensor.from_numpy(smoothed_rot).float().to(device)
...
class OneEuroFilter:
  ...
  def process(self, x, print_inter=False):
    ...
    if isinstance(edx, float):
        ...
        cutoff = self.mincutoff + self.beta * np.abs(edx)
    elif isinstance(edx, mindspore.Tensor):
        cutoff = self.mincutoff + self.beta * ops.abs(edx)
    if print_inter:
...
def smooth_results(filters, body_pose=None, body_shape=None, cam=None):
    if body_pose is not None:
        ...
        body_pose = ops.cat([global_rot, filters['smpl_thetas'].process(body_pose[3:])], 0)
...
def get_tracked_ids(detections, tracked_objects):
    tracked_ids_out = mindspore.Tensor([obj.id for obj in tracked_objects])
    tracked_points = mindspore.Tensor([obj.last_detection.points[0] for obj in tracked_objects])
    org_points = mindspore.Tensor([obj.points for obj in detections])
...
def get_tracked_ids3D(detections, tracked_objects):
    tracked_ids_out = mindspore.Tensor([obj.id for obj in tracked_objects])
    tracked_points = mindspore.Tensor([obj.last_detection.points for obj in tracked_objects])
    org_points = mindspore.Tensor([obj.points for obj in detections])
...
def convert_kp2d_from_input_to_orgimg(kp2ds, offsets):
    ...
    leftTop = ops.stack([crop_trbl[:,3]-pad_trbl[:,3], crop_trbl[:,0]-pad_trbl[:,0]],1)
...
def convert_cam_to_3d_trans(cams, weight=2.):
    ...
    trans3d = ops.stack([dx, dy, depth], 1)*weight
    return trans3d
...
def batch_orth_proj(X, camera, mode='2d', keep_dim=False):
    ...
    X_camed += camera[:, :, 1:]
    if keep_dim:
        X_camed = ops.cat([X_camed, X[:,:,2].unsqueeze(-1)],-1)
    return X_camed

def vertices_kp3d_projection(outputs, meta_data=None, presp=False):
    vertices, j3ds = outputs['verts'], outputs['j3d']
    verts_camed = batch_orth_proj(vertices, outputs['cam'], mode='3d',keep_axis=True)
    ...
    cam_trans = estimate_translation(predicts_j3ds, predicts_pj2ds, \
                                focal_length=443.4, img_size=mindspore.Tensor([512,512])).to(vertices.device)
...
def estimate_translation_cv2(joints_3d, joints_2d, focal_length=600, img_size=mindspore.Tensor([512.,512.]), proj_mat=None, cam_dist=None):
    if proj_mat is None:
        camK = np.eye(3)
...
def estimate_translation_np(joints_3d, joints_2d, joints_conf, focal_length=600, img_size=mindspore.Tensor([512.,512.]), proj_mat=None):
    ...
    if proj_mat is None:
        # focal length
        f = mindspore.Tensor([focal_length,focal_length])
        # optical center
        center = img_size/2.
    else:
        f = mindspore.Tensor([proj_mat[0,0],proj_mat[1,1]])
        center = proj_mat[:2,2]
    ...
    # least squares
    Q = mindspore.Tensor([F*np.tile(mindspore.Tensor([1,0]),num_joints), F*np.tile(mindspore.Tensor([0,1]),num_joints), O-np.reshape(joints_2d,-1)]).T
...
def estimate_translation(joints_3d, joints_2d, pts_mnum=4,focal_length=600, proj_mats=None, cam_dists=None,img_size=mindspore.Tensor([512.,512.])):
    """Find camera translation that brings 3D joints joints_3d closest to 2D the corresponding joints_2d.
    Input:
    ...
    """
    if ops.is_tensor(joints_3d):
        joints_3d = joints_3d.detach().cpu().numpy()
    if ops.is_tensor(joints_2d):
    ...
    for i in range(joints_3d.shape[0]):
        ...
        try:
            ...
            trans[i] = estimate_translation_np(S_i[valid_mask], joints_i[valid_mask], valid_mask[valid_mask].astype(mindspore.float32), 
                focal_length=focal_length, img_size=imgsize, proj_mat=proj_mats[i])

    return mindspore.Tensor.from_numpy(trans).float()
...
def rot6d_to_rotmat(x):
    ...
    b1 = ops.L2Normalize()(x[:, :, 0], axis=1, eps=1e-6)
...
def rotation_matrix_to_quaternion(rotation_matrix, eps=1e-6):
    ...
    q /= ops.sqrt(t0_rep * mask_c0 + t1_rep * mask_c1 +  # noqa
...
def determine_device(gpu_id):
    if gpu_id != -1:
        device = mindspore.set_context('cuda:{}'.format(gpu_id))
    else:
        device = mindspore.set_context('cpu')
    return device
