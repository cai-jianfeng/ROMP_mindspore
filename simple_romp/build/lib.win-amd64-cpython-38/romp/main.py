...
class ROMP(nn.Cell):
    ...
    def _build_model_(self):
        if not self.settings.onnx:
            model = ROMPv1().eval()
            model.load_state_dict(mindspore.load_checkpoint(self.settings.model_path, map_location=self.tdevice))
    ...
    def single_image_forward(self, image):
        ...
        if self.settings.onnx:
            center_maps, params_maps = self.ort_session.run(None, {'image':input_image.numpy().astype(mindspore.float32)})
            center_maps, params_maps = mindspore.Tensor.from_numpy(center_maps).to(self.tdevice), mindspore.Tensor.from_numpy(params_maps).to(self.tdevice)
        else:
            center_maps, params_maps = self.model(input_image.to(self.tdevice))
        params_maps[:, 0] = ops.pow(1.1, params_maps[:, 0])
    ...
    def temporal_optimization(self, outputs, signal_ID):
        ...
        if self.settings.show_largest:
            max_id = ops.argmax(outputs['cam'][:,0])
            ...
            outputs['track_ids'] = mindspore.Tensor(tracked_ids).astype(np.int32)
        return outputs

    @time_cost('ROMP')
    def construct(self, image, signal_ID=0, **kwargs):
