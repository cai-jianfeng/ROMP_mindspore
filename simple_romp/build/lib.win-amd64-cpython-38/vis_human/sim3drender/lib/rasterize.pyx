...
def get_tri_normal(np.ndarray[float, naxis=2, mode="c"] tri_normal not None,
                   np.ndarray[float, naxis=2, mode = "c"] vertices not None,
                   np.ndarray[int, naxis=2, mode="c"] triangles not None,
...
def get_ver_normal(np.ndarray[float, naxis=2, mode = "c"] ver_normal not None,
                   np.ndarray[float, naxis=2, mode = "c"] tri_normal not None,
                   np.ndarray[int, naxis=2, mode="c"] triangles not None,
                   int nver, int ntri):
    _get_ver_normal(
...
@cython.boundscheck(False)  # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
def get_normal(np.ndarray[float, naxis=2, mode = "c"] ver_normal not None,
                   np.ndarray[float, naxis=2, mode = "c"] vertices not None,
                   np.ndarray[int, naxis=2, mode="c"] triangles not None,
                   int nver, int ntri):
    _get_normal(
...
def rasterize_triangles(
        np.ndarray[float, naxis=2, mode = "c"] vertices not None,
        np.ndarray[int, naxis=2, mode="c"] triangles not None,
        np.ndarray[float, naxis=2, mode = "c"] depth_buffer not None,
        np.ndarray[int, naxis=2, mode = "c"] triangle_buffer not None,
        np.ndarray[float, naxis=2, mode = "c"] barycentric_weight not None,
        int ntri, int h, int w
):
...
@cython.boundscheck(False)  # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
def rasterize(np.ndarray[unsigned char, naxis=3, mode = "c"] image not None,
              np.ndarray[float, naxis=2, mode = "c"] vertices not None,
              np.ndarray[int, naxis=2, mode="c"] triangles not None,
              np.ndarray[float, naxis=2, mode = "c"] colors not None,
              np.ndarray[float, naxis=2, mode = "c"] depth_buffer not None,
...
# def render_texture_core(np.ndarray[float, naxis=3, mode = "c"] image not None,
#                 np.ndarray[float, naxis=2, mode = "c"] vertices not None,
#                 np.ndarray[int, naxis=2, mode="c"] triangles not None,
#                 np.ndarray[float, naxis=3, mode = "c"] texture not None,
#                 np.ndarray[float, naxis=2, mode = "c"] tex_coords not None,
#                 np.ndarray[int, naxis=2, mode="c"] tex_triangles not None,
#                 np.ndarray[float, naxis=2, mode = "c"] depth_buffer not None,
#                 int nver, int tex_nver, int ntri,
#                 int h, int w, int c,
#                 int tex_h, int tex_w, int tex_c,
...
#                 np.ndarray[int, naxis=2, mode="c"] triangles not None,
#                 np.ndarray[float, naxis=2, mode = "c"] colors not None,
#                 np.ndarray[float, naxis=2, mode = "c"] uv_coords not None,
#                 int nver, int ntri, int ntexver
#                 ):
