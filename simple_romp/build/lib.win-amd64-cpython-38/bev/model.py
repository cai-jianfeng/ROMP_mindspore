...
def get_cam3dmap_anchor(FOV, centermap_size):
    ...
    map_coord_range_each_level = (mindspore.Tensor([2/64., 25/64., 3/64., 2/64.], dtype=mindspore.float32) * centermap_size).astype(np.int)
...
def convert_cam_params_to_centermap_coords(cam_params, cam3dmap_anchor):
    ...
    if len(cam_params) != 0:
        center_coords[:,0] = ops.argmin(ops.abs(cam_params[:,[0]].repeat(1, scale_num) - cam3dmap_anchors), axis=1).float()/128 * 2. - 1.
    
    return center_coords

def denormalize_center(center, size=128):
    center = (center+1)/2*size
    center = ops.clamp(center, 1, size-1).long()
    return center
...
class BEVv1(nn.Cell):
    ...
    def _build_head(self):
        ...
        self.cam3dmap_anchor = mindspore.Tensor.from_numpy(get_cam3dmap_anchor(60, self.outmap_size)).float()
    ...
    def _make_bv_center_layers(self, input_channels, output_channels):
        ...
        self.bv_pre_layers = nn.SequentialCell(
                    ...
                    nn.ReLU(),\
                    nn.Conv2d(in_channels=num_channels,out_channels=num_channels,kernel_size=3,stride=1,pad_mode='pad', padding=1),\
                    nn.BatchNorm2d(num_channels, momentum=BN_MOMENTUM),\
                    nn.ReLU(),\
        ...
        inter_channels = 512
        self.bv_out_layers = nn.SequentialCell(
    ...
    def _make_3D_map_refiner(self):
        self.center_map_refiner = nn.SequentialCell(BasicBlock_3D(self.output_cfg['NUM_CENTER_MAP'], self.output_cfg['NUM_CENTER_MAP']))
        self.cam_map_refiner = nn.SequentialCell(BasicBlock_3D(self.output_cfg['NUM_CAM_MAP'], self.output_cfg['NUM_CAM_MAP']))
    
    def fv_conditioned_bv_estimation(self, x, center_maps_fv, cam_maps_offset):
        img_feats = self.bv_pre_layers(x)
        summon_feats = ops.cat([center_maps_fv, cam_maps_offset, img_feats], 1).view(img_feats.size(0), -1, self.outmap_size)
    ...
    def mesh_parameter_regression(self, fv_f, cams_preds, pred_batch_ids):
        ...
        params_preds = ops.cat([cams_preds, params_preds], 1)
...
def export_model_to_onnx_static():
    model = BEVv1().cuda()
    state_dict = mindspore.load_checkpoint('/home/yusun/CenterMesh/trained_models/BEV_review.pth')
    ...
    image = mindspore.Tensor.from_numpy(image)[None].cuda().float()
    mindspore.export(model, (image),
                      save_file, 
                      file_format='ONNX',
                      input_names=['image'],
...
if __name__ == '__main__':
    ...
    input_image = mindspore.Tensor.from_numpy(image).cuda()[None]
