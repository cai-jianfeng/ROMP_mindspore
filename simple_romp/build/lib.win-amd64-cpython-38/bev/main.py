...
class BEV(nn.Cell):
    ...
    def _build_model_(self):
        ...
        model.load_state_dict(mindspore.load_checkpoint(self.settings.model_path, map_location=self.tdevice), strict=False)
    ...
    def process_long_image(self, full_image, show_patch_results=False):
        ...
        for cid in range(len(crop_boxes)):
            ...
            if this_outs is not None:
                ...
                projection = body_mesh_projection2image(this_outs['joints'], this_outs['cam'], vertices=this_outs['verts'], input2org_offsets=mindspore.Tensor([0, ch, 0, cw, ch, cw]))
