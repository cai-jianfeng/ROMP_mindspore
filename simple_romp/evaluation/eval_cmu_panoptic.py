...
def get_results():
    ...
    #J_regressor_h36m = mindspore.load_checkpoint(default_eval_settings.smpl_path)['J_regressor_h36m17']
    results = {}
    ...
    for image_path in progress_bar(file_list):
        ...
        #pred_kp3ds = ops.matmul(J_regressor_batch, pred_vertices)
...
def determine_visible_person(kp2ds, width, height):
    ...
    return mindspore.Tensor(visible_person_id), mindspore.Tensor(kp2d_vis)
...
def load_gts():
    ...
    new_annots = {}
    J24_TO_H36M = mindspore.Tensor([14, 3, 4, 5, 2, 1, 0, 16, 12, 17, 18, 9, 10, 11, 8, 7, 6])
    ...
    for img_name in annots:
        ...
        for inds, (kp2d, kp3d) in enumerate(zip(kp2ds, kp3ds)):
            ...
            kp3d = kp3d[:,:3]
            kp3d[invis_3dkps] = -2.
            kp3d[:13] += mindspore.Tensor([0,0.06,0.03])
            kp_3ds[inds] = kp3d
...
def match_2d_greedy(
    ...
    errors_per_pair_list = mindspore.Tensor(errors_per_pair_list)
    ...
    bestMatch = mindspore.Tensor(bestMatch)
