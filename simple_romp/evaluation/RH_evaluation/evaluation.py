...
def _calc_relative_depth_error_weak_(pred_depths, depth_ids, reorganize_idx, age_gts=None, matched_mask=None):
    ...
    for b_ind in ops.unique(valid_inds):
        ...
        if did_num > 1:
            ...
            # error_dict['correct'].append([(ops.abs(eq_dists)<thresh).sum().item(), (cd_dists<-thresh).sum().item(), (fd_dists>thresh).sum().item()])
...
def get_results(depth_relative, missed_age_ids, dr_thresh=0.2, miss_fine=0.3):
    eval_results = {}
    eq_dists = ops.cat(depth_relative['eq'], 0)
    cd_dists = ops.cat(depth_relative['cd'], 0)
    fd_dists = ops.cat(depth_relative['fd'], 0)
    eq_age_ids = ops.cat(depth_relative['eq_age'], 0)
    cd_age_ids = ops.cat(depth_relative['cd_age'], 0)
    fd_age_ids = ops.cat(depth_relative['fd_age'], 0)
    dr_age_ids = ops.cat([eq_age_ids, cd_age_ids, fd_age_ids], 0)
    dr_all = mindspore.Tensor([len(eq_dists), len(cd_dists), len(fd_dists), len(missed_age_ids)*miss_fine])
...
class RH_Evaluation(object):
    ...
    def match_kp2ds(self):
        ...
        for img_name in self.annots.keys():
            ...
            for idx,annot in enumerate(annots):
                vbox = mindspore.Tensor(annot['bbox'])
                if 'kp2d' in annot:
                    if annot['kp2d'] is not None:
                        joint = mindspore.Tensor(annot['kp2d']).reshape((-1,3))
                        ...
                        gt_inds.append(idx)
            gt_kp2ds = mindspore.Tensor(gt_kp2ds)
            
            if img_name not in self.results:
                self.no_predictions(len(gt_inds))
                self.missed_ids[img_name] = mindspore.Tensor(gt_inds)
                continue
            results = self.results[img_name]
            if isinstance(results, list):
                pred_kp2ds = mindspore.Tensor([r['kp2ds'] for r in results])
            ...
            if len(bestMatch)>0:
                ...
                bestMatch[:,1] = mindspore.Tensor([gt_inds[ind] for ind in gt_ids])
            if len(misses)>0:
                self.missed_ids[img_name] = mindspore.Tensor([gt_inds[ind] for ind in misses])
    ...
    def calc_error(self):
        ...
        for img_name, match_mat in self.match_results.items():
            ...
            annots = self.annots[img_name]
            depth_ids = mindspore.Tensor.from_numpy(mindspore.Tensor([annots[ind]['depth_id'] for ind in gt_ids]))
            age_ids = mindspore.Tensor.from_numpy(mindspore.Tensor([annots[ind]['age'] for ind in gt_ids]))

            results = self.results[img_name]
            if isinstance(results, list):
                pred_depth = mindspore.Tensor.from_numpy(mindspore.Tensor([results[ind]['trans'][2] for ind in pred_ids]))
            elif isinstance(results, dict):
                pred_depth = mindspore.Tensor.from_numpy(results['trans'][pred_ids,2])
            default_organize_idx = ops.zeros(len(depth_ids))

            mPCKh = _calc_matched_PCKh_(mindspore.Tensor.from_numpy(self.kp2ds['gts'][img_name]).float(), mindspore.Tensor.from_numpy(self.kp2ds['preds'][img_name]).float(), ops.ones(len(self.kp2ds['gts'][img_name])).bool())
            self.mPCKh.append(mPCKh)
            matched_mask = ops.ones(len(depth_ids)).bool()
        ...
        for img_name, missed_id in self.missed_ids.items():
            annots = self.annots[img_name]
            missed_age_ids.append(mindspore.Tensor([annots[ind]['age'] for ind in missed_id]))
        missed_age_ids = mindspore.Tensor.from_numpy(np.concatenate(missed_age_ids, 0))
        
        print_results = []
        all_mPCKh = ops.cat(self.mPCKh).mean()
