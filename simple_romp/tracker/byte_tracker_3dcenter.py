...
class Tracker(object):
    ...
    def update(self, trans3D, scores):
        ...
        dists = matching.euclidean_distance(strack_pool, detections, axis=4)
        ...
        dists = matching.euclidean_distance(
            r_tracked_stracks, detections_second, axis=4)
        ...
        output_results = mindspore.Tensor([mindspore.Tensor([*track.trans, track.track_id]) for track in self.tracked_stracks if track.is_activated])
...
def get_tracked_ids_byte(tracking_points, tracked_objects):
    tracked_ids_out = mindspore.Tensor([obj[4] for obj in tracked_objects])
    tracked_points = mindspore.Tensor([obj[:4] for obj in tracked_objects])

    tracked_ids, tracked_bbox_ids = [], []
    for tid, tracked_point in enumerate(tracked_points):
        dist = mindspore.Tensor([np.linalg.norm(tracked_point-org_point) for org_point in tracking_points])
...
def remove_duplicate_stracks(stracksa, stracksb, dist_thresh=0.15):
    ...
    pdist = matching.euclidean_distance(stracksa, stracksb, axis=2)
