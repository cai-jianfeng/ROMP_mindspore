...
def get_image_split_plan(image, overlap_ratio=0.46):
    ...
    return mindspore.Tensor(crop_box).astype(np.int32)
...
def exclude_boudary_subjects(outputs, drop_boundary_ratio, ptype='left', torlerance=0.05):
    ...
    remove_subjects(outputs, ops.where(drop_mask)[0])
    ...
    for key in keys:
        if key not in all_outputs:
            ...
            if key in ['center_map']:
                all_outputs[key] = ops.cat([all_outputs[key], outputs[key]],3)
                continue
            if key in ['center_map_3d']:
                all_outputs[key] = ops.cat([all_outputs[key], outputs[key]],2)
                continue
            all_outputs[key] = ops.cat([all_outputs[key], outputs[key]],0)
