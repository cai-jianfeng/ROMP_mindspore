...
class CenterMap3D(object):
    ...
    def prepare_parsing(self):
        ...
        self.maxpool3d = nn.MaxPool3d(5, 1, (5-1)//2)
    ...
    def generate_kernels(self, kernel_size_list):
        ...
        for kernel_size in set(kernel_size_list):
            ...
            pool_group[kernel_size] = nn.MaxPool2d(kernel_size, 1, (kernel_size-1)//2)
    ...
    def parse_3dcentermap(self, center_maps):
        ...
        center_zyxs = ops.stack([topk_zs[mask].long(), topk_ys[mask].long(), topk_xs[mask].long()]).permute((1,0)).long()
...
def perspective_projection(points, translation=None,rotation=None, 
    ...
    if isinstance(points,np.ndarray):
        points = mindspore.Tensor.from_numpy(points).float()
    if isinstance(translation,np.ndarray):
        translation = mindspore.Tensor.from_numpy(translation).float()
    batch_size = points.shape[0]
    K = ops.zeros([batch_size, 3, 3], device=points.device)
    K[:,0,0] = focal_length
    K[:,1,1] = focal_length
    K[:,2,2] = 1.
    ...
    # projected_points = ops.einsum('bij,bkj->bki', K, projected_points)[:, :, :-1]
    projected_points = ops.matmul(projected_points.contiguous(), K.contiguous())
...
def denormalize_cam_params_to_trans(normed_cams, positive_constrain=False):
    ...
    trans_XY_normed = ops.flip(normed_cams[:, 1:],[1])
    ...
    trans = ops.cat([trans_XY, depth], 1)
...
def body_mesh_projection2image(j3d_preds, cam_preds, vertices=None, input2org_offsets=None):
    ...
    if vertices is not None:
        ...
        projected_outputs['verts_camed'] = ops.cat([projected_outputs['verts_camed'], vertices[:,:,[2]]], -1)
...
def remove_subjects(outputs, removed_subj_inds):
    ...
    remove_mask = ops.ones(N).bool()
    remove_mask[removed_subj_inds] = False
    left_subj_inds = ops.where(remove_mask)[0].tolist()
...
def suppressing_redundant_prediction_via_projection(outputs, img_shape, thresh=16, conf_based=False):
    ...
    repeat_subj_inds = ops.where(pj2d_dist_mat_normalized<thresh)
    if len(repeat_subj_inds)>0:
        ...
        if conf_based:
            ...
            removed_subj_inds = ops.where(center_confs[repeat_subj_inds[0]]<center_confs[repeat_subj_inds[1]], repeat_subj_inds[0], repeat_subj_inds[1])
        else:
            removed_subj_inds = ops.where(person_scales[repeat_subj_inds[0]]<person_scales[repeat_subj_inds[1]], repeat_subj_inds[0], repeat_subj_inds[1])
...
def remove_outlier(outputs, relative_scale_thresh=3, scale_thresh=0.25): #0.3
    ...
    trans_dist_mat = ops.norm(trans_diff, p=2, axis=-1)
    # drop the least and the largest dist 
    trans_dist_mat = ops.Sort()(trans_dist_mat).values[:,1:-1]
    ...
    removed_subj_inds = ops.where(outlier_mask)[0]
...
def pack_params_dict(params_pred):
    ...
    params_dict['body_pose'] = ops.cat([params_dict['body_pose'], ops.zeros(N,6).to(params_dict['body_pose'].device)],1)
    params_dict['smpl_thetas'] = ops.cat([params_dict['global_orient'], params_dict['body_pose']], 1)

    return {'cam': params_dict['cam'], 'smpl_thetas': params_dict['smpl_thetas'], 'smpl_betas': params_dict['smpl_betas']}

class SMPLA_parser(nn.Cell):
    ...
    def construct(self, betas=None, thetas=None, root_align=True):
        baby_mask = betas[:,10] > self.baby_thresh
        if baby_mask.sum()>0:
            adult_mask = ~baby_mask
            person_num = len(thetas)
            verts, joints = ops.zeros(person_num, 6890, 3, device=thetas.device).float(), ops.zeros(person_num, 54+17, 3, device=thetas.device).float()
