...
def convert_type(obj):
    if isinstance(obj, tuple) or isinstance(obj, list):
        return mindspore.Tensor(obj, dtype=mindspore.float32)[None, :]
    return obj

def get_normal(vertices, triangles):
    normal = np.zeros_like(vertices, dtype=mindspore.float32)
...
def rasterize(
    ...
    buffer = np.zeros((height, width), dtype=mindspore.float32) - 1e8

    if colors.dtype != mindspore.float32:
        colors = colors.astype(mindspore.float32)
...
class Sim3DR(object):
    ...
    def render(
        self, vertices, triangles, bg, color=mindspore.Tensor([[1, 0.6, 0.4]]), texture=None
    ):
        normal = get_normal(vertices, triangles)

        # 2. lighting
        light = np.zeros_like(vertices, dtype=mindspore.float32)
        # ambient component
        if self.intensity_ambient > 0:
            light += self.intensity_ambient * mindspore.Tensor(color)
    ...
    def __call__(self, verts_list, triangles, bg, mesh_colors=mindspore.Tensor([[1, 0.6, 0.4]])):
