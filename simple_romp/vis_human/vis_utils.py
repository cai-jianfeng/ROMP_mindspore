...
color_list = mindspore.Tensor([[.7, .7, .6],[.7, .5, .5],[.5, .5, .7],  [.5, .55, .3],[.3, .5, .55],  \
...
def rotate_view_weak_perspective(verts, rx=30, ry=0, img_shape=[512,512], expand_ratio=1.2, bbox3D_center=None, scale=None):
    ...
    verts_rot = ops.einsum('bij,kj->bik', verts, Rx_mat)
    verts_rot = ops.einsum('bij,kj->bik', verts_rot, Ry_mat)
    ...
    rendered_image_center = mindspore.Tensor([[[w / 2, h / 2]]]).to(device).type(verts_aligned.dtype)
    
    if scale is None:
        # To ensure all vertices are visible, we need to rescale the vertices
        scale = 1 / (expand_ratio * ops.abs(ops.div(verts_aligned[:,:,:2], rendered_image_center)).max()) 
...
def rotate_view_perspective(verts, rx=30, ry=0, FOV=60, bbox3D_center=None, depth=None):
    ...
    if depth is None:
        ...
        dist_min = ops.abs(verts_aligned.view(-1, 3).min(0).values)
        dist_max = ops.abs(verts_aligned.view(-1, 3).max(0).values)
        z = dist_max[:2].max() / np.tan(np.radians(FOV/2)) + dist_min[2]
        depth = mindspore.Tensor([[[0, 0, z]]], device=device)    
...
def rendering_mesh_rotating_view(vert_trans, renderer, triangles, image, background, internal=5):
    ...
    go_up = np.sin(np.arange(change_time).astype(mindspore.float32)/change_time) * 1
    go_down = np.sin(np.arange(change_time).astype(mindspore.float32)/change_time - 1) * 1
    ...
    camera_pose[:3,3] = mindspore.Tensor([0,cam_height,0]) # translation
    ...
    for azimuth_angle, elevation_angle in zip(azimuth_angles, elevation_angles):
        ...
        rendered_image, rend_depth = renderer(verts_rotated.cpu().numpy(), triangles, background, mesh_colors=mindspore.Tensor([[0.9, 0.9, 0.8]]), camera_pose=camera_pose)
        result_imgs.append(rendered_image)
    
    return result_imgs


color_table_default = mindspore.Tensor([
    [0.4, 0.6, 1], # blue
    [0.8, 0.7, 1], # pink
    [0.1, 0.9, 1], # cyan
    [0.8, 0.9, 1], # gray
...
def mesh_color_left2right(trans, color_table=None):
    left2right_order = ops.sort(trans[:,0].cpu()).indices.numpy()
    color_inds = np.arange(len(trans))
    color_inds[left2right_order] = np.arange(len(trans))
    if color_table is None:
        color_table = color_table_default
    return mindspore.Tensor([color_table[ind % len(color_table)] for ind in color_inds])

tracking_color_list = mindspore.Tensor(
        [
            0.000, 0.447, 0.741,
            0.850, 0.325, 0.098,
            0.929, 0.694, 0.125,
            ...
            0.857, 0.857, 0.857,
            1.000, 1.000, 1.000
        ]).astype(mindspore.float32).reshape((-1,3))

def mesh_color_trackID(track_ids, color_table=None):
    if color_table is None:
        color_table = tracking_color_list
    return mindspore.Tensor([color_table[tid % len(color_table)] for tid in track_ids])
...
def draw_skeleton(image, pts, bones=smpl24_connMat, cm=None, label_kp_order=False,r=8):
    ...
    if bones is not None:
        set_colors = mindspore.Tensor([cm for i in range(len(bones))]).astype(np.int)
...
class Plotter3dPoses:
    ...
    def encircle_plot(self, pose_3ds, bones, colors=[(255, 255, 255)], img=None):
        ...
        encircle_origin = mindspore.Tensor([[0.165, 0.165], [0.165, 0.495], [0.165, 0.825],\
                                    [0.495, 0.165], [0.495, 0.495], [0.495, 0.825],\
                                    [0.825, 0.165], [0.825, 0.495], [0.825, 0.825]], dtype=mindspore.float32) * mindspore.Tensor(self.canvas_size)[None]
    ...
    def _get_rotation(self, theta, phi):
        ...
        return mindspore.Tensor([
            ...
            [ 0,                       -cos(phi)]
        ], dtype=mindspore.float32)  # transposed