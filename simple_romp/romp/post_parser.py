...
def convert_cam_to_3d_trans2(j3ds, pj3d): 
    ...
    cam_trans = estimate_translation(predicts_j3ds, predicts_pj2ds, \
                                focal_length=443.4, img_size=mindspore.Tensor([512,512])).to(j3ds.device)
    return cam_trans
...
class SMPL_parser(nn.Cell):
    def __init__(self, model_path):
        super(SMPL_parser, self).__init__()
        self.smpl_model = SMPL(model_path)
    
    def construct(self, outputs, root_align=False):
...
def parsing_outputs(center_maps, params_maps, centermap_parser):
    ...
    parsed_results['center_preds'] = ops.stack([flat_inds%64, flat_inds//64],1) * 512 // 64
