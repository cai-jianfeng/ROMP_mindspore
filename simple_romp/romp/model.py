...
def get_coord_maps(size=128):
    xx_ones = ops.ones([1, size], dtype=mindspore.int32)
    xx_ones = xx_ones.unsqueeze(-1)

    xx_range = ms_np.arange(size, dtype=mindspore.int32).unsqueeze(0)
    xx_range = xx_range.unsqueeze(1)

    xx_channel = ops.matmul(xx_ones, xx_range)
    xx_channel = xx_channel.unsqueeze(-1)

    yy_ones = ops.ones([1, size], dtype=mindspore.int32)
    yy_ones = yy_ones.unsqueeze(1)

    yy_range = ms_np.arange(size, dtype=mindspore.int32).unsqueeze(0)
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                ...
                for j in range(i+1-num_branches_pre):
                    ...
                    conv3x3s.append(nn.SequentialCell(
                        nn.Conv2d(
                            inchannels, outchannels, 3, 2, 1, has_bias=False),
...
class ROMPv1(nn.Cell):
    ...
    def construct(self, image):
        ...
        x = ops.cat((x, self.coordmaps.to(x.device).repeat(x.shape[0],1,1,1)), 1)
        ...
        #cam_maps[:, 0] = ops.pow(1.1,cam_maps[:, 0]) # not supported by tensorRT
...
def export_model_to_onnx_static():
    model = ROMPv1().cuda()
    state_dict = mindspore.load_checkpoint('/home/yusun/.romp/ROMP.pkl')
    model.load_state_dict(state_dict)
    save_file = '/home/yusun/.romp/ROMP.onnx'

    image = ops.rand(1,512,512,3).cuda()
...
def test_model():
    ...
    outputs = model(ops.rand(1,512,512,3).cuda())
...
def test_onnx():
    ...
    import numpy as np
    cost_time = []
    image = cv2.imread('/home/yusun/CenterMesh/simple_romp/test/ages_croped.png').astype(mindspore.float32)[None]
    for _ in range(200):
