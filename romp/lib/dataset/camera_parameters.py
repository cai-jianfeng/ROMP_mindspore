            ...
            for i, cam in enumerate(cameras):
                ...
                for k, v in cam.items():
                    if k not in ['id', 'res_w', 'res_h']:
                        cam[k] = mindspore.Tensor(v, dtype='float32')
...
def project_to_2d(X, camera_params):
    ...
    XX = ops.clamp(X[..., :2] / X[..., 2:], min=-1, max=1)
    r2 = ops.sum(XX[..., :2]**2, axis=len(XX.shape)-1, keepaxis=True)

    radial = 1 + ops.sum(k * ops.cat((r2, r2**2, r2**3), axis=len(r2.shape)-1), axis=len(r2.shape)-1, keepaxis=True)
    tan = ops.sum(p*XX, axis=len(XX.shape)-1, keepaxis=True)
