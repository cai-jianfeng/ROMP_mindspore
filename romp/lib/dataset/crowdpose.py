...
def Crowdpose(base_class=default_mode):
    class Crowdpose(Base_Classes[base_class]):
        ...
        def get_image_info(self,index):
            ...
            kp2ds = mindspore.Tensor([self.map_kps(kp2d, self.kp2d_mapper) for kp2d in self.annots[img_name]])
            valid_mask_2d = mindspore.Tensor([[True,True,True] for _ in range(person_num)])
            valid_mask_3d = mindspore.Tensor([self.default_valid_mask_3d for _ in range(person_num)])
            ...
            img_info = {'imgpath': imgpath, 'image': image, 'kp2ds': kp2ds, 'track_ids': np.arange(len(kp2ds)),\
                    ...
                    'img_size': image.shape[:2], 'ds': 'crowdpose'}
...
if __name__ == '__main__':
    dataset = Crowdpose()(train_flag=True)
