...
def MPII(base_class=default_mode):
    class MPII(Base_Classes[base_class]):
        def __init__(self, train_flag=True, regress_smpl=False, **kwargs):
            ...
            self.const_box = [mindspore.Tensor([0,0]),mindspore.Tensor([256,256])]
        ...
        def get_image_info(self,index):
            ...
            valid_mask_2d, valid_mask_3d = mindspore.Tensor(valid_mask_2d), mindspore.Tensor(valid_mask_3d)
            ...
            if img_name in self.eft_annots and self.use_eft:
                ...
                bbox_center_list = mindspore.Tensor(bbox_center_list)
                for inds, kp2d in enumerate(kp2ds):
                    ...
                    valid_mask_3d[inds, :4] = mindspore.Tensor([self.regress_smpl, True, True, False])

            kp2ds, params = mindspore.Tensor(kp2ds), mindspore.Tensor(params)
        ...
        def load_eft_annots(self, annot_file_path):
            ...
            for eft_data in annots:
                ...
                pred_camera = mindspore.Tensor(eft_data['parm_cam'])
                pred_betas = np.reshape(mindspore.Tensor( eft_data['parm_shape'], dtype=mindspore.float32), (10) )     #(10,)
                pred_pose_rotmat = np.reshape( mindspore.Tensor( eft_data['parm_pose'], dtype=mindspore.float32), (24,3,3)  )        #(24,3,3)
        ...
        def _get_db(self):
            ...
            for a in anno:
                ...
                c = mindspore.Tensor(a['center'], dtype=mindspore.float32)
                s = mindspore.Tensor([a['scale'], a['scale']], dtype=mindspore.float32)
                ...
                c = c - 1
                joints = mindspore.Tensor(a['joints'])
                joints[:, 0:2] = joints[:, 0:2] - 1
                assert len(joints) == 16, 'joint num diff: {} vs {}'.format(len(joints),16)

                joints_vis = np.zeros((16,3),dtype=mindspore.float32)
                joints_vis[:, 0:2] = joints[:, 0:2]
                joints_vis[:, -1] = mindspore.Tensor(a['joints_vis'])
