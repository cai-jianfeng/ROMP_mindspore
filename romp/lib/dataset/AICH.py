...
def AICH(base_class=default_mode):
    class AICH(Base_Classes[base_class]):
        ...
        def _ai_ch_to_lsp(self, pts):
            ...
            pts = mindspore.Tensor(pts, dtype = np.float).reshape(14, 3).copy()
            pts[:, 2] = (3.0 - pts[:, 2]) / 2.0
        ...
        def get_image_info(self,index):
            ...
            valid_mask_2d = mindspore.Tensor([[True,False,True] for _ in range(len(kp2ds))])
            valid_mask_3d = mindspore.Tensor([self.default_valid_mask_3d for _ in range(len(kp2ds))])
            
            # vmask_2d | 0: kp2d/bbox | 1: track ids | 2: detect all people in image
            # vmask_3d | 0: kp3d | 2: smpl global orient | 3: smpl body pose | 4: smpl body shape | 5: smpl verts | 6: depth
            img_info = {'imgpath': imgpath, 'image': image, 'kp2ds': mindspore.Tensor(kp2ds), 'track_ids': np.arange(len(kp2ds)),\
