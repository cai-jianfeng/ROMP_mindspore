...
def MuCo(base_class=default_mode):
    class MuCo(Base_Classes[base_class]):
        ...
        def get_image_info(self, index):
            ...
            camMats = mindspore.Tensor([[f[0],0,c[0]],[0,f[1],c[1]],[0,0,1]])
            ...
            img_info = {'imgpath': imgpath, 'image': image, 'kp2ds': kp2ds, 'track_ids': np.arange(len(kp2ds)),\
                    'vmask_2d': mindspore.Tensor(valid_mask_2d), 'vmask_3d': mindspore.Tensor(valid_mask_3d),\
        ...
        def pack_data_augmented(self, annots_file_path):
            ...
            for iid in db.imgs.keys():
                ...
                f = img["f"]
                c = img["c"]
                intrinsic = mindspore.Tensor([f,c])

                # crop the closest person to the camera
                ann_ids = db.getAnnIds(img_id)
                anns = db.loadAnns(ann_ids)
                kp2d = mindspore.Tensor([ann['keypoints_img'] for ann in anns])
                kp3d = mindspore.Tensor([ann['keypoints_cam'] for ann in anns])
...
def read_calibration(calib_file, vid_list):
    ...
    for vid_i in vid_list:
        K = mindspore.Tensor([float(s) for s in content[vid_i*7+5][11:-2].split()])
        K = np.reshape(K, (4, 4))
        RT = mindspore.Tensor([float(s) for s in content[vid_i*7+6][11:-2].split()])
