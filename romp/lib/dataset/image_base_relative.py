...
class Image_base_relative(Image_base):
    ...
    def get_item_single_frame(self,index, augment_cfgs=None):
        ...
        rot_flip = mindspore.Tensor([position_augments[0], position_augments[1]]) if position_augments is not None else mindspore.Tensor([0,0])
        ...
        if args().perspective_proj:
            ...
            input_data.update({'cams': cam_params,'cam_mask': cam_mask, 'root_trans':root_trans,\
                'centermap_3d':centermap_3d.float(),'valid_centermap3d_mask':mindspore.Tensor([valid_centermap3d_mask]).bool()})

        if args().learn_relative:
            # age; gender; depth level; body type
            depth_info = self._organize_depth_info_(info, used_person_inds)
            input_data.update({'depth_info': mindspore.Tensor.from_numpy(depth_info).long()})
    ...
    def generate_dynamic_depth(self, cam_params, cam_mask, bboxes_hw_norm, all_person_detected_mask):
        radius_list = mindspore.Tensor(_calc_radius_(bboxes_hw_norm)).long()
        person_scales = ops.zeros(self.max_person).long()
    ...
    def _organize_depth_info_(self, info, used_person_inds):
        ...
        if 'depth' in info:
            if len(info['depth']) != 0:
                prepared_info[:len(used_person_inds)] = mindspore.Tensor(info['depth'])[used_person_inds]
        return prepared_info

    def _organize_kid_shape_offsets_(self, info, used_person_inds):
        kid_shape_offsets_processed = ops.ones(self.max_person).float()*-1
        if 'kid_shape_offsets' in info:
            kid_shape_offsets_processed[:len(used_person_inds)] = mindspore.Tensor.from_numpy(info['kid_shape_offsets'][used_person_inds]).float()
    ...
    for _,r in enumerate(dataloader):
        ...
        if with_smpl and r['valid_masks'][0,0,4]:
            ...
            if test_projection_part and r['cam_mask'][inds].sum()>0:
                ...
                image = mindspore.Tensor(image).astype(np.uint8)
                if len(person_centers) == len(trans):
                    for pinds, person_center in enumerate(person_centers):
                        y,x = person_center.astype(np.int)
                        if y>0 and x>0:
                            cv2.circle(mindspore.Tensor(image), (x,y), 6, [0,0,255],-1)
