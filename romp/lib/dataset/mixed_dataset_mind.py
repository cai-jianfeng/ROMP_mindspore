...
dataset_dict = {'h36m': H36M, 'mpii': MPII, 'coco': COCO14, 'posetrack':Posetrack21, 'aich':AICH, 'pw3d':PW3D, 'up':UP, 'crowdpose':Crowdpose, 'crowdhuman':CrowdHuman, \
    ...
    'cmup':CMU_Panoptic_eval,'internet':Internet, 'relative_human': Relative_human, 'agora':AGORA,'flag3d':FLAG3D}
...
class MixedDataset(Dataset):
    def __init__(self, datasets_used, sample_prob_dict, loading_modes=None, max_length=args().batch_size*10000, **kwargs):
        ...
        expect_length = (mindspore.Tensor(self.lengths)/mindspore.Tensor(self.partition)).astype(np.int32)
        dataset_info_table.add_row(['Expected length']+expect_length.asnumpy().tolist())
        self.partition = mindspore.Tensor(self.partition).cumsum()
        dataset_info_table.add_row(['Accum. Prob.']+self.partition.astype(np.float16).asnumpy().tolist())
