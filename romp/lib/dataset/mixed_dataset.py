...
dataset_dict = {'h36m': H36M, 'mpii': MPII, 'coco': COCO14, 'posetrack': Posetrack21, 'aich': AICH, 'pw3d': PW3D,
                'up': UP, 'crowdpose': Crowdpose, 'crowdhuman': CrowdHuman,
                'lsp': LSP, 'mpiinf': MPI_INF_3DHP, 'mpiinf_val': MPI_INF_3DHP_VALIDATION,
...
class MixedDataset:
    def __init__(self, datasets_used, sample_prob_dict, loading_modes=None, max_length=args().batch_size * 10000,
                 **kwargs):
        # datasets_used = [h36m,mpiinf,coco,mpii,lsp,muco,crowdpose]
        # sample_prob_dict = [
        #   h36m: 0.2
        #   mpiinf: 0.16
        #   coco: 0.2
        ...
        if not 1.001 > sum(sample_prob_dict.values()) > 0.999:
            print(
                'CAUTION: The sum of sampling rates is supposed to be 1, while currently we have {}, \n please properly set the sample_prob_dict {} in config.yml' \
    ...
    def __getitem__(self, index):
        ...
        if (upper_bound - lower_bound) * self.total_length < self.lengths[dataset_id]:
            ...
            index_sample = int(min(self.lengths[dataset_id] * sample_prob + random.randint(0, omit_internal),
                                   self.lengths[dataset_id] - 1))
        else:
