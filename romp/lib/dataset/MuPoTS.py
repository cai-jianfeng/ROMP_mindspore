...
def MuPoTS(base_class=default_mode):
    class MuPoTS(Base_Classes[base_class]):
        ...
        def get_image_info(self, index):
            ...
            kp2ds, kp3ds = mindspore.Tensor(kp2ds), mindspore.Tensor(kp3ds)
            root_trans = kp3ds[:,self.root_inds].mean(1)
            valid_masks = mindspore.Tensor([self._check_kp3d_visible_parts_(kp3d) for kp3d in kp3ds])
            kp3ds -= root_trans[:,None]
            kp3ds[~valid_masks] = -2.
            fx, fy, cx, cy = self.annots[img_name][3]
            camMats = mindspore.Tensor([[fx, 0, cx], [0, fy, cy], [0, 0, 1]])
            person_num = len(kp2ds)

            vis_masks = []
            for kp2d in kp2ds:
                vis_masks.append(_check_visible(kp2d,get_mask=True))
            kp2ds = np.concatenate([kp2ds, mindspore.Tensor(vis_masks)[:,:,None]],2)

            img_info = {'imgpath': imgpath, 'image': image, 'kp2ds': kp2ds, 'track_ids': None,\
                    'vmask_2d': mindspore.Tensor([[True,True,True] for _ in range(person_num)]), 'vmask_3d': mindspore.Tensor([[True,False,False,False,False,True] for _ in range(person_num)]),\
        ...
        def pack_data(self):
            ...
            for aid in db.anns.keys():
                ...
                intrinsic_params = mindspore.Tensor([fx, fy, cx, cy])
            ...
            for seq_id in range(1, 21):
                ...
                seq_valid_flag = np.zeros((frame_num, subject_num),dtype=np.bool_)
                seq_joint_occlusion = np.zeros((frame_num, subject_num, 17),dtype=np.bool_)
...
def calculate_score(output_path, annot_path, thr=250):
    ...
    for n in range(len(output)):
        ...
        for ann_id,ann in enumerate(anns):
            if ann['is_valid'] == 0:
                continue
            gt_root = mindspore.Tensor(ann['keypoints_cam'])
            root_idx = 14
...
def process_bbox(bbox, width, height):
    ...
    if w*h > 0 and x2 >= x1 and y2 >= y1:
        bbox = mindspore.Tensor([x1, y1, x2-x1, y2-y1])
    else:
        return None
...
def flip(tensor, dims):
    if not isinstance(dims, (tuple, list)):
        dims = [dims]
    indices = [ms_np.arange(tensor.shape[dim] - 1, -1, -1,
...
def vis_3d_skeleton(kpt_3d, kpt_3d_vis, kps_lines, filename=None):
    ...
    cmap = plt.get_cmap('rainbow')
    colors = [cmap(i) for i in np.linspace(0, 1, len(kps_lines) + 2)]
    colors = [mindspore.Tensor((c[2], c[1], c[0])) for c in colors]

    for l in range(len(kps_lines)):
        i1 = kps_lines[l][0]
        i2 = kps_lines[l][1]
        x = mindspore.Tensor([kpt_3d[i1,0], kpt_3d[i2,0]])
        y = mindspore.Tensor([kpt_3d[i1,1], kpt_3d[i2,1]])
        z = mindspore.Tensor([kpt_3d[i1,2], kpt_3d[i2,2]])
...
def vis_3d_multiple_skeleton(kpt_3d, kpt_3d_vis, kps_lines, filename=None):
    ...
    for l in range(len(kps_lines)):
        ...
        for n in range(person_num):
            x = mindspore.Tensor([kpt_3d[n,i1,0], kpt_3d[n,i2,0]])
            y = mindspore.Tensor([kpt_3d[n,i1,1], kpt_3d[n,i2,1]])
            z = mindspore.Tensor([kpt_3d[n,i1,2], kpt_3d[n,i2,2]])
