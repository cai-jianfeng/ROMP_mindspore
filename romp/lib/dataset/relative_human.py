    ...
    class Relative_human(Base_Classes[base_class]):
        ...
        def get_image_info(self,index):
            ...
            for idx,annot in enumerate(annots):
                ...
                #vbox[2:] += vbox[:2]
                #fbox = mindspore.Tensor(annot['bbox_wb']) if 'bbox_wb' in annot else vbox
                fbox = vbox
                joint = mindspore.Tensor([fbox[:2], fbox[2:], vbox[:2], vbox[2:]])
                if 'kp2d' in annot:
            ...
            valid_mask_2ds, valid_mask_3ds = mindspore.Tensor(valid_mask_2ds), mindspore.Tensor(valid_mask_3ds)
            depth_info = mindspore.Tensor(depth_info)
