...
def LSP(base_class=default_mode):
    class LSP(Base_Classes[base_class]):
        ...
        def load_data(self):
            ...
            joints = scio.loadmat(os.path.join(root_dir,'joints.mat'))['joints'].transpose(2,0,1).astype(mindspore.float32)
        ...
        def get_image_info(self,index):
            ...
            params, valid_mask_3d = [], mindspore.Tensor([self.default_valid_mask_3d])
            if img_name in self.eft_annots and self.use_eft:
                eft_annot = self.eft_annots[img_name]
                bbox_center, pose, betas = eft_annot[0]
                params = mindspore.Tensor([np.concatenate([pose[:66], betas])])
                valid_mask_3d[0,:4] = mindspore.Tensor([self.regress_smpl, True, True, False])
                
            kp3ds = self.regress_kp3d_from_smpl(params)
            valid_mask_2d = mindspore.Tensor([[True,False,False]])
