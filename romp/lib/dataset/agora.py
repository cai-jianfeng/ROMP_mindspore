...
def AGORA(base_class=default_mode):
    class AGORA(Base_Classes[base_class]):
        ...
        def get_image_info(self,index,total_frame=None):
            ...
            valid_mask = np.where(mindspore.Tensor([annot['isValid'] for annot in annots]))[0]
            if len(valid_mask) ==0:
                print(img_name, 'lack valid person')
                valid_mask = mindspore.Tensor([0])
            ...
            img_info = {'imgpath': imgpath, 'image': image, 'kp2ds': kp2ds, 'track_ids': track_ids,\
                    'vmask_2d': mindspore.Tensor([[True,True,True] for _ in range(person_num)]), 'vmask_3d': mindspore.Tensor([[True,False,True,True,True,True] for _ in range(person_num)]),\
            ...
            if 'relative' in base_class:
                ...
                if ages.sum()>0:
                    img_info['kid_shape_offsets'] = mindspore.Tensor([annot['betas'][0,10] if annot['betas'].shape[-1]==11 else 0 for annot in annots])
...
def pack_data(vertex_save_dir, data_folder, split, annots_path):
    ...
    for af_ind, annot_file in enumerate(all_annot_paths):
        ...
        for annot_ind, annot_dict in enumerate(annot_dicts):
            ...
            pimg_annot = {
                    'cam_locs':mindspore.Tensor([annot_dict['camX'],annot_dict['camY'],annot_dict['camZ'],annot_dict['camYaw']]),
                    'trans':mindspore.Tensor([annot_dict['X'],annot_dict['Y'],annot_dict['Z'],annot_dict['Yaw']]).transpose((1,0)),
                    'props':mindspore.Tensor([annot_dict['gender'],annot_dict['kid'],annot_dict['occlusion'],annot_dict['age'],annot_dict['ethnicity']]),
