...
default_mode = args().image_loading_mode  # image


def H36M(base_class=default_mode):
    # Base_Classes[base_class] = image_base
    class H36M(Base_Classes[base_class]):
        ...
        def __init__(self, train_flag=True, split='train', regress_smpl=True, **kwargs):
            ...
            self.camMat_views = [mindspore.Tensor([[intrinsic['focal_length'][0], 0, intrinsic['center'][0]],
                                                   [0, intrinsic['focal_length'][1], intrinsic['center'][1]],
                                                   [0, 0, 1]]) for intrinsic in h36m_cameras_intrinsic_params]
            ...
            self.cam_distortions = [np.array([*intrinsic['radial_distortion'][:2],
                                              *intrinsic['tangential_distortion'],
                                              intrinsic['radial_distortion'][2]])
                                    for intrinsic in h36m_cameras_intrinsic_params]

            if self.regress_smpl:  # 进入
            ...
            self.kps_vis = (self.joint_mapper != -1).astype(mindspore.float32)[:, None]
        ...
        def get_image_info(self, index, total_frame=None):
            ...
            root_rotation = mindspore.Tensor(info['cam'])[smpl_randidx]
            pose = mindspore.Tensor(info['poses'])[smpl_randidx]
            pose[:3] = root_rotation
            beta = mindspore.Tensor(info['betas'])
            ...
            img_info = {'imgpath': imgpath, 'image': image, 'kp2ds': kp2ds, 'track_ids': track_ids, \
                        'vmask_2d': mindspore.Tensor([[True, True, True]]),
                        'vmask_3d': mindspore.Tensor([[True, True, True, True, True, True]]), \
