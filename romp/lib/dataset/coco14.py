...
def COCO14(base_class=default_mode):
    class COCO14(Base_Classes[base_class]):
        ...
        def get_image_info(self,index):
            ...
            #mask = self.get_exclude_mask(anno, index)[:,:,np.newaxis].astype(mindspore.float32)
            ...
            if img_name in self.eft_annots and self.use_eft:
                ...
                cdist = mindspore.Tensor([np.linalg.norm(bbox_center_list-self._calc_center_(kp2d)[:2][None], axis=-1) for kp2d in kp2ds])
                matches = []
                ...
                for kid, pid in matches:
                    ...
                    valid_mask_3d[kid] = mindspore.Tensor([self.regress_smpl,True,True,False,False,True])
        ...
        def get_joints(self, anno):
            ...
            for i, obj in enumerate(anno):
                joint = mindspore.Tensor(obj['keypoints']).reshape([-1, 3])
                if joint[:, -1].sum()<1:
                    continue
                joints.append(joint)
            
            return mindspore.Tensor(joints)
