...
def Posetrack21(base_class=default_mode):
    class Posetrack21(Base_Classes[base_class]):
        ...
        def get_image_info(self,index):
            ...
            bboxes = mindspore.Tensor(self.annots[img_name][1])
            for ind, joint in enumerate(self.annots[img_name][0]):
                ...
                if valid_2dpose:
                    ...
                    x,y,w,h = bboxes[ind]
                    kp2ds.append(mindspore.Tensor([[x,y], [x+w,y+h], [x,y], [x+w,y+h]]))
                    valid_mask_2d.append([False,True,True])
                valid_mask_3d.append(self.default_valid_mask_3d)

            track_ids = mindspore.Tensor(self.annots[img_name][2])
...
def get_joints_ids(anno):
    ...
    for i, obj in enumerate(anno):
        bbox = mindspore.Tensor(obj['bbox'])
        joint = mindspore.Tensor(obj['keypoints']).reshape([-1, 3])
        valid_mask = ((joint[:, 0]>0) * (joint[:, 1]>0)).astype(np.bool_)
    ...
    return mindspore.Tensor(joints), mindspore.Tensor(bboxes), track_ids
...
def pack_annots(annots_dir, annots_file_path):
    ...
    for annot_path in glob.glob(os.path.join(annots_dir, '*.json')):
        ...
        for index, img_id in enumerate(img_ids):
            ...
            annots[file_name] = [joints, bboxes, mindspore.Tensor(img_person_ids)]
            if index%1000==0:
