...
def FLAG3D(base_class=default_mode):
    class FLAG3D(Base_Classes[base_class]):
        def __init__(self,train_flag = False, split='train', mode='vibe', regress_smpl=True, **kwargs):
            ...
            self.img_folder = "/mnt/disk_1/yutao_intern/images"             # "/mnt/data/images"
            self.kpts_folder_3d = "/mnt/disk_1/yutao_intern/smpl_24_joints" # "/mnt/data/smpl_24_joints"
            self.kpts_folder_2d = "/mnt/disk_1/yutao_intern/2d_joints/"     # "/mnt/data/2d_joints/"
            self.param_folder = "/mnt/disk_1/yutao_intern/smpl_param"       # "/mnt/data/smpl_param"
            self.mode = mode
        ...
        def map_kps(self,joint_org,maps=None):
            kps = joint_org[maps.asnumpy()].copy()
            kps[maps.asnumpy()==-1] = -2.
            return kps
        ...
        def get_image_info(self, index):
            ...
            imgpath=os.path.join(os.path.join(self.img_folder,self.train_names[seq_id]),"{:06}.jpg".format(frame_id))
            image = cv2.imread(imgpath)[:, :, ::-1]

            kp3d = np.load(os.path.join(self.kpts_folder_3d,inverse_symbol(self.train_names[seq_id])[:4]+"C001"+self.train_names[seq_id])+".npy")[frame_id]
            ...
            param = joblib.load(os.path.join(self.param_folder,inverse_symbol(self.train_names[seq_id])[:4]+"C001"+self.train_names[seq_id])+".pkl")
            ...
            # valid_mask_2d, valid_mask_3d,params = mindspore.Tensor(valid_mask_2d), mindspore.Tensor(valid_mask_3d),mindspore.Tensor(params)
            valid_mask_2d, valid_mask_3d,params = np.asarray(valid_mask_2d), np.asarray(valid_mask_3d), np.asarray(params)
            # print(valid_mask_2d, valid_mask_3d,params)
...
def pack_data(data3d_dir, annots_path):
    ...
    for path_gt in paths_gt:
        ...
        all_params[video_name]['betas'] = mindspore.Tensor(data_gt['betas'])
        for i in range(len(genders)):
            ...
            trans_params = mindspore.Tensor(data_gt['trans'])
