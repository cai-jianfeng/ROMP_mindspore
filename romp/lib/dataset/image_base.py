...
from utils.augments import get_image_cut_box, process_image, calc_aabb, flip_kps, rot_imgplane, pose_processing, \
    Synthetic_occlusion, convert_bbox2scale
...
class Image_base:
    def __init__(self, train_flag=True, regress_smpl=False, **kwargs):
        ...
        if not os.path.isdir(self.data_folder):
            self.data_folder = '/home/jianfeng_intern/ROMP/ROMP_code/dataset/'
        ...
        if train_flag:
            ...
            if args().color_jittering_ratio > 0:
                self.color_jittering = mindspore.dataset.vision.RandomColorAdjust(brightness=0.1, contrast=0.1,
    ...
    def get_item_single_frame(self, index):
        ...
        verts_processed, valid_masks[:, 6], root_trans_processed, valid_masks[:, 7] = self.process_verts(info['verts'],
                                                                                                         info['root_trans'],
                                                                                                         used_person_inds,
                                                                                                         augments=position_augments,
                                                                                                         valid_mask_verts=info['vmask_3d'][:, 4],
                                                                                                         valid_mask_depth=info['vmask_3d'][:, 5])

        rot_flip = np.array([position_augments[0], position_augments[1]]) if position_augments is not None else np.array([0, 0])
        input_data = {
            'image': mindspore.Tensor.from_numpy(dst_image).float(),
            'image_org': mindspore.Tensor.from_numpy(org_image),
            'full_kp2d': mindspore.Tensor.from_numpy(full_kp2ds).float(),
            'person_centers': mindspore.Tensor.from_numpy(person_centers).float(),
            'subject_ids': mindspore.Tensor.from_numpy(subject_ids).long(),
            'centermap': centermap.float(),
            'kp_3d': mindspore.Tensor.from_numpy(kp3d).float(),
            'verts': mindspore.Tensor.from_numpy(verts_processed).float(),
            'params': mindspore.Tensor.from_numpy(params).float(),
            'valid_masks': mindspore.Tensor.from_numpy(valid_masks).bool(),
            'root_trans': mindspore.Tensor.from_numpy(root_trans_processed).float(),
            'offsets': mindspore.Tensor.from_numpy(offsets).float(),
            'rot_flip': mindspore.Tensor.from_numpy(rot_flip).float(),
            'all_person_detected_mask': mindspore.Tensor([all_person_detected_mask]).bool(),
        ...
        if self.train_flag:
            ...
            input_data['img_scale'] = mindspore.Tensor([img_scale]).float()

        if args().learn_2dpose:
            input_data.update({'heatmap': mindspore.Tensor.from_numpy(heatmap).float()})
        if args().learn_AE:
            input_data.update({'AE_joints': mindspore.Tensor.from_numpy(AE_joints).long()})

        if args().perspective_proj:
            root_trans, cam_params, cam_mask = self._calc_normed_cam_params_(full_kp2ds, kp3d, valid_masks[:, 1],
                                                                             info['ds'])
            centermap_3d, valid_centermap3d_mask = self.generate_centermap_3d(person_centers, cam_params, cam_mask,
    ...
    def generate_centermap_3d(self, person_centers, cam_params, cam_mask, bboxes_hw_norm, all_person_detected_mask):
        ...
        centermap_3d, valid_centermap3d_mask = self.CM.generate_centermap_3dheatmap_adaptive_scale(
            valid_pc_zyx[:, [2, 1, 0]])
    ...
    def process_smpl_params(self, params, used_person_inds, augments=None, valid_mask_smpl=None):
        ...
        for inds, used_id in enumerate(used_person_inds):
            if valid_mask_smpl[used_id].sum() > 0:
                ...
                if augments is not None:
                    theta = pose_processing(theta, augments[0], augments[1], valid_grot=smpl_flag[inds, 0],
                                            valid_pose=smpl_flag[inds, 1])
                params_processed[inds] = np.concatenate([theta, beta])

        return params_processed, smpl_flag

    def process_verts(self, verts, root_trans, used_person_inds, augments=None, valid_mask_verts=None,
                      valid_mask_depth=None):
    ...
    def _calc_normed_cam_params_(self, full_kp2ds, kp_3ds, kp3d_flag, ds, thresh=50):
        cam_params = ops.ones(self.max_person, 3, dtype=mindspore.float32) * -2
        cam_mask = ops.zeros(self.max_person, dtype=mindspore.bool_)
        root_trans = ops.ones(self.max_person, 3, dtype=mindspore.float32) * -2
        ...
        if valid_mask.sum() > 0:
            cam_params[kp3d_flag] = mindspore.Tensor.from_numpy(normed_cams).float()
    ...
    def add_cam_parameters(self, input_data, info):
        if 'camMats' in info:
            input_data['camMats'] = mindspore.Tensor.from_numpy(info['camMats']).float()
        else:
            input_data['camMats'] = mindspore.Tensor([[args().focal_length, 0, info['img_size'][1] / 2.], \
                                                      [0, args().focal_length, info['img_size'][0] / 2.], \
                                                      [0, 0, 0]])
        if 'camDists' in info:
            input_data['camDists'] = mindspore.Tensor.from_numpy(info['camDists']).float()
        else:
            input_data['camDists'] = ops.zeros(5).float()
        return input_data
    ...
    for _, r in enumerate(dataloader.create_tuple_iterator()):
        ...
        for inds in range(2):
            ...
            if test_projection_part and r['cam_mask'][inds].sum() > 0:
                ...
                pred_keypoints_2d = perspective_projection(kp3d_tp, translation=pred_cam_t,
                                                           focal_length=args().focal_length, normalize=False) + 512 // 2
            ...
            person_centers_onmap = ((r['person_centers'][inds].numpy() + 1) / 2.0 * (args().centermap_size - 1)).astype(
                np.int)
            positive_position = ops.stack(ops.where(r['centermap'][inds, 0] == 1)).permute(1, 0)
        ...
        if with_smpl and r['valid_masks'][0, 0, 4]:
            ...
            if test_projection_part:
                ...
                pred_keypoints_2d = perspective_projection(joints, translation=trans, focal_length=args().focal_length,
                                                           normalize=False) + 512 // 2

                render_img = \
                visualizer.visualize_renderer_verts_list([verts.cuda()], trans=[trans.cuda()], images=image[None])[0]
                image_kp2d_projection_smpl24 = draw_skeleton_multiperson(render_img.copy(), pred_keypoints_2d,
                                                                         bones=bones[:23], cm=cm[:23],
                                                                         label_kp_order=True)
                image_kp2d_projection_extra30 = draw_skeleton_multiperson(render_img.copy(), pred_keypoints_2d,
                                                                          bones=bones[23:], cm=cm[23:],
                                                                          label_kp_order=True)
                # cv2.imwrite('{}/op25_regressed_{}.jpg'.format(save_dir,_), image_kp2d_projection)
                rendered_img_bv = \
                visualizer.visualize_renderer_verts_list([verts.cuda()], trans=[trans.cuda()], bird_view=True,
                                                         auto_cam=True)[0]
                cv2.imwrite('{}/mesh_{}.png'.format(save_dir, _), np.concatenate(
                    [render_img, rendered_img_bv, image_kp2d_projection_smpl24, image_kp2d_projection_extra30], 1))
            else:
                verts[:, :, 2] += 2
