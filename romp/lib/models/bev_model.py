...
class BEV(Base):
    ...
    def _build_head(self):
        ...
        self.cam3dmap_anchor = mindspore.Tensor.from_numpy(constants.get_cam3dmap_anchor(args().FOV, 128)).float() # args().centermap_size
    ...
    def parsing_trans3D(self, center_maps_3d, cam_maps_3d):
        detection_flag = mindspore.Tensor([False for _ in range(len(center_maps_3d))]).cuda()
...
if __name__ == '__main__':
    ...
    outputs = model.feed_forward({'image':ops.zeros(2,512,512,3).cuda()})
    print_dict(outputs)