...
# import mindspore # import torchvision.models.resnet as resnet
# import mindspore # import torchvision.transforms.functional as F
...
class ResNet_50(nn.Cell):
    ...
    def load_pretrain_params(self):
        if os.path.exists(args().resnet_pretrain):
            success_layer = copy_state_dict(self.state_dict(), mindspore.load_checkpoint(args().resnet_pretrain), prefix = '', fix_loaded=True)

    def image_preprocess(self, x):
        x = BHWC_to_BCHW(x)/255.
        #x = F.normalize(x, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225],).contiguous() # for pytorch version>1.8.0
        x = ops.stack(list(map(lambda x:mindspore.dataset.vision.Normalize(x, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225],inplace=False),x)))
...
if __name__ == '__main__':
    ...
    a=model(ops.rand(2,512,512,3).cuda())
    for i in a:
        print(i.shape)