    ...
    for key in params[0]:
        if isinstance(params[0][key], np.ndarray):
            if len(params[0][key].shape)>0:
                params_batch[key] = mindspore.Tensor.from_numpy(np.concatenate([param[key] for param in params], 0)).float()
    return params_batch
...
class VisOpen3DSocket(BaseSocket):
    def __init__(self, host, port, cfg) -> None:
        ...
        for key, val in cfg.extra.items():
            ...
            trans = mindspore.Tensor(val['transform']).reshape(4, 4)
        ...
        zero_params = {
            'poses': ops.zeros(1, 72).float(),
            'betas': ops.zeros(1, 10).float(),
        }
    ...
    def add_human(self, zero_params):
        ...
        if args().mesh_cloth in constants.wardrobe or args().mesh_cloth=='random':
            ...
            mesh_color = mindspore.Tensor(constants.mesh_color_dict[args().mesh_cloth])/255.
    ...
    def set_camera(cfg, camera_pose):
        ...
        rot_x = mindspore.Tensor([
            [1., 0., 0.],
            [0., ct, -st],
            [0, st, ct]
        ])
        rot_z = mindspore.Tensor([
            [cp, -sp, 0],
            [sp, cp, 0.],
            [0., 0., 1.]
        ])
    ...
    def init_camera(self, camera_pose):
        ...
        init_param.extrinsic = mindspore.Tensor(camera_pose)
        ctr.convert_from_pinhole_camera_parameters(init_param) 

    def get_camera(self):
        ctr = self.vis.get_view_control()
        init_param = ctr.convert_to_pinhole_camera_parameters()
        return mindspore.Tensor(init_param.extrinsic)
    ...
    def main(self, datas):
        ...
        for data in datas:
            for key in data.keys():
                if key == 'id':
                    continue
                data[key] = mindspore.Tensor(data[key])
...
'''
    ...
    def filter_human(self, datas):
        datas_new = []
        for data in datas:
            kpts3d = mindspore.Tensor(data['keypoints3d'])
