...
class Visualizer(object):
    def __init__(self, resolution=(512,512), result_img_dir = None, with_renderer=False):
        ...
        if with_renderer:
            ...
            if args().model_version>=4:
                ...
                R,T = look_at_view_transform(dist=5,elev=80,azim=180,at=mindspore.Tensor([[0.,0.,3]]))
        ...
        self.mesh_color = (mindspore.Tensor([[[0.65098039, 0.74117647, 0.85882353]]])*255).long()
        self.color_table = mindspore.Tensor([[255,0,0], [0,255,0], [0,0,255], [0,255,255], [255,0,255], [255,255,0], [128,128,0], [0,128,128], [128,0,128]])
    ...
    def visualize_renderer_verts_list(self, verts_list, faces_list=None, images=None, bird_view=False, auto_cam=False, cam_params=None,\
                                            colors=mindspore.Tensor([.9, .9, .8]), trans=None, thresh=0.):
        ...
        rendered_imgs = ops.cat(rendered_imgs, 0).cpu().numpy()
    ...
    def show_verts_on_imgs(self, outputs, meta_data, all_inds, org_imgs, mesh_colors=None,img_names=None, put_org=True, plot_dict=None, save2html=True, drop_texts=False):
        ...
        if self.perps_proj:
            ...
            if mesh_colors is None:
                mesh_colors = mindspore.Tensor([.9, .9, .8])
                if 'Age_preds' in outputs and 'kid_offsets_pred' in outputs:
                    ...
                    mesh_colors = [mindspore.Tensor([self.age_color_dict[age] for age in age_preds]) for age_preds in class_preds]
    ...
    def visulize_result(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, save2html=True, **kwargs):
        ...
        img_names = mindspore.Tensor(meta_data['imgpath'])[img_inds_org]
        ...
        for vis_name in show_items:
            ...
            if vis_name == 'j3d' and detection_flag:
                ...
                if save2html:
                    ...
                    plot_dict['j3d'] = {'figs':mindspore.Tensor(skeleton_3ds), 'type':'skeleton'}
...
def calc_auto_cam_params(verts):
    ...
    height = 20
    R,T = look_at_view_transform(dist=height,elev=280,azim=0,at=mindspore.Tensor([[cy, cx, cz]]))
    #return (R, T, fov)
    return (R, T, xyz_ranges)


def make_heatmaps(image, heatmaps):
    heatmaps = ops.interpolate(heatmaps[None],size=image.shape[:2],mode='bilinear')[0]
...
def draw_skeleton(image, pts, bones=None, cm=None, label_kp_order=False,r=3):
    ...
    if bones is not None:
        if cm is None:
            set_colors = mindspore.Tensor([[255,0,0] for i in range(len(bones))]).astype(np.int)
        else:
            if len(bones)>len(cm):
