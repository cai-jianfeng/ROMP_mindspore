...
def write_common_results(dumpname=None, results=[], keys=[], fmt='%2.3f'):
    ...
    for idata, data in enumerate(results):
        ...
        for key in keys:
            ...
            # output[key] = mindspore.Tensor2string(data[key], max_line_width=1000, separator=', ', formatter=format_out)
...
def encode_image(image):
    ...
    data = mindspore.Tensor(img_encode) # numpy array로 안바꿔주면 ERROR
