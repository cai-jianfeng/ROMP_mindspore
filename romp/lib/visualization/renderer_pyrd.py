...
class Renderer(object):
    ...
    def __init__(self, focal_length=600, height=512, width=512,**kwargs):
        ...
        self.camera_center = mindspore.Tensor([width / 2., height / 2.])
    ...
    def __call__(self, verts, faces, colors=None,focal_length=None,camera_pose=None,**kwargs):
        ...
        if isinstance(faces, mindspore.Tensor):
        ...
        for n in range(num_people):
            ...
            light_pose = np.eye(4)
            light_pose[:3, 3] = mindspore.Tensor([0, -1, 1]) + trans
            scene.add(light, pose=light_pose)
            light_pose[:3, 3] = mindspore.Tensor([0, 1, 1]) + trans
            scene.add(light, pose=light_pose)
            light_pose[:3, 3] = mindspore.Tensor([1, 1, 2]) + trans
            scene.add(light, pose=light_pose)
