...
class MeshRendererWithDepth(nn.Cell):
    def __init__(self, rasterizer, shader):
        super().__init__()
        self.rasterizer = rasterizer
        self.shader = shader

    def construct(self, meshes_world, **kwargs) -> mindspore.Tensor:
...
class Renderer(nn.Cell):
    def __init__(self, resolution=(512,512), perps=True, R=None, T=None, fov=args().FOV, use_gpu=args().gpu!='-1', with_depth=False):
        ...
        if T is None:
            T = mindspore.Tensor([[0., 0., 0.]])

        if self.perps:
            # Initialize a camera.
            self.cameras = FoVPerspectiveCameras(R=R, T=T, fov=fov, device=self.device)
            self.lights = PointLights(ambient_color=((0.56, 0.56, 0.56),),location=mindspore.Tensor([[0., 0., 0.]]), device=self.device)
        else:
...
def set_mesh_color(verts_rgb, colors):
    if colors is None:
        colors = mindspore.Tensor(mesh_color_table['neutral'])
...
def get_renderer(test=False,**kwargs):
    ...
    if test:
        ...
        np_v_template = mindspore.Tensor.from_numpy(mindspore.Tensor(model['v_template'])).cuda().float()[None]
        face = mindspore.Tensor.from_numpy(model['f'].astype(np.int32)).cuda()[None]
