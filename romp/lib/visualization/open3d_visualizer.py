...
def convert_trans_scale(trans):
    trans *= mindspore.Tensor([0.4, 0.6, 0.7])
    return trans

class Open3d_visualizer(object):
    def __init__(self, multi_mode=False):
        self.window_size = mindspore.Tensor([1280,1080])
        #self.window_size = mindspore.Tensor([720,720])
        self._init_viewer_()
        if not multi_mode:
    ...
    def prepare_single_person_scene(self):
        ...
        cam_location = mindspore.Tensor([0,0,0])
