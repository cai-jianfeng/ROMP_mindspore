...
class Vedo_visualizer(object):
    def __init__(self):
        if args().character == 'smpl':
            self.faces = mindspore.load_checkpoint(args().smpl_model_path)['f'].numpy()
        elif args().character == 'nvxia':
            params_dict = np.load(os.path.join(args().nvxia_model_path, 'nvxia.npz'), allow_pickle=True)
            self.faces = mindspore.Tensor([mindspore.Tensor(face) for face in params_dict['polygons']])
    ...
    def plot_multi_meshes_batch(self, vertices, cam_params, meta_data, reorganize_idx, save_img=True, interactive_show=False, rotate_frames=[]):
        ...
        for inds, img_id in enumerate(np.unique(reorganize_idx)):
            single_img_verts_inds = mindspore.Tensor(np.where(reorganize_idx==img_id)[0])
...
#         else:
#             self.mesh_color = mindspore.Tensor(constants.mesh_color_dict[args().webcam_mesh_color])/255.
...
#         self.vp.show(mesh,viewup=mindspore.Tensor([0,-1,0]))
...
#         points = mesh.points()
#         new_points = mindspore.Tensor(points)
