...
class HeatmapGenerator():
    ...
    def batch_process(self,batch_joints):
        ...
        batch_joints = (ops.cat([batch_joints,vis],-1).unsqueeze(1)+1)/2 * self.output_res 
        heatmaps = []
        for joints in batch_joints:
            heatmaps.append(mindspore.Tensor.from_numpy(self.single_process(joints)))
        return ops.stack(heatmaps).cuda()
...
class JointsGenerator():
    ...
    def batch_process(self,batch_joints):
        ...
        return mindspore.Tensor.from_numpy(mindspore.Tensor(joints_processed)).long().cuda()
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        x = ops.rand(1,num_joints,2).cuda()*2-1
        x[0,:2] = -2.
