...
class CenterMap(object):
    ...
    def prepare_parsing(self):
        ...
        self.maxpool3d = nn.MaxPool3d(kernel_size=5, stride=1, pad_mode="pad", padding=(5 - 1) // 2)
    ...
    def generate_kernels(self, kernel_size_list):
        ...
        for kernel_size in set(kernel_size_list):
            ...
            pool_group[kernel_size] = nn.MaxPool2d(kernel_size=kernel_size, stride=1, pad_mode='pad', padding=(kernel_size - 1) // 2)
    ...
    def generate_centermap_heatmap_adaptive_scale(self, center_locs, bboxes_hw_norm, occluded_by_who=None, **kwargs):
            ...
            for cur_idx, occluded_idx in enumerate(occluded_by_who):
                if occluded_idx > -1:
                    ...
                    if dist_onmap < least_dist:
                        offset = np.abs(((radius_list[occluded_idx] + radius_list[
                            cur_idx] + 1) / self.size * 2 - dist_onmap) / dist_onmap) \
        ...
        heatmap = mindspore.Tensor.from_numpy(heatmap)
        return heatmap
    ...
    def generate_centermap_3dheatmap_adaptive_scale_batch(self, batch_center_locs, radius=3, depth_num=None,
                                                          device='cuda:0'):
        if depth_num is None:
            depth_num = int(self.size // 2)
        heatmap = ops.zeros((len(batch_center_locs), depth_num, self.size, self.size))
        ...
        for bid, center_locs in enumerate(batch_center_locs):
            for cid, center in enumerate(center_locs):
                ...
                gaussian_patch = gaussian3D(w=diameter, h=diameter, d=diameter,
                                            center=(diameter // 2, diameter // 2, diameter // 2), s=float(diameter) / 6,
                                            device=device)

                xa, ya, za = int(max(0, center[0] - diameter // 2)), int(max(0, center[1] - diameter // 2)), int(
                    max(0, center[2] - diameter // 2))
                xb, yb, zb = int(min(center[0] + diameter // 2, self.size - 1)), int(
                    min(center[1] + diameter // 2, self.size - 1)), int(min(center[2] + diameter // 2, depth_num - 1))
                ...
                heatmap[bid, za:zb + 1, ya:yb + 1, xa:xb + 1] = ops.max(
                    ops.cat(tuple([
    ...
    def generate_centermap_3dheatmap_adaptive_scale(self, center_locs, depth_num=None, device='cpu'):
        ...
        if depth_num is None:
            depth_num = int(self.size // 2)
        heatmap = ops.zeros((depth_num, self.size, self.size))
        ...
        for cid, center in enumerate(center_locs):
            ...
            gaussian_patch = gaussian3D(w=width, h=height, d=depth,
                                        center=(width // 2, height // 2, depth // 2), s=float(diameter) / 6,
                                        device=device)

            xa, ya, za = int(max(0, center[0] - width // 2)), int(max(0, center[1] - height // 2)), int(
                max(0, center[2] - depth // 2))
            xb, yb, zb = int(min(center[0] + width // 2, self.size - 1)), int(
                min(center[1] + height // 2, self.size - 1)), int(min(center[2] + depth // 2, depth_num - 1))
            ...
            heatmap[za:zb + 1, ya:yb + 1, xa:xb + 1] = ops.max(
                ops.cat(tuple([
    ...
    def multi_channel_nms(self, center_maps):
        ...
        center_maps_max = ops.max(ops.cat(center_map_pooled, 0), 0)[0]
    ...
    def parse_centermap_mask(self, center_map):
        center_map_bool = ops.argmax(center_map, 1).bool()
        center_idx = ops.stack(ops.where(center_map_bool))
        center_idx = ops.swapaxes(center_idx, axis0=1, axis1=0)
        return center_idx
    ...
    def parse_centermap_heatmap_adaptive_scale_batch(self, center_maps, top_n_people=None):
        ...
        topk_scores, topk_inds = ops.topk(center_map_nms.reshape((b, c, -1)), K)
        topk_inds = topk_inds % (h * w)
        topk_ys = ops.div(topk_inds.long(), w).float()
        topk_xs = (topk_inds % w).int().float()
        # get all topk in in a batch
        topk_score, index = ops.topk(topk_scores.reshape((b, -1)), K)
        # div by K because index is grouped by K(C x K shape)
        topk_clses = ops.div(index.long(), K)
    ...
    def parse_3dcentermap_heatmap_adaptive_scale_batch(self, center_maps, top_n_people=None):
        ...
        if top_n_people is not None:
            ...
            mask = topk_score > self.conf_thresh
        batch_ids = ops.where(mask)[0]
        center_zyxs = ops.stack([topk_zs[mask], topk_ys[mask], topk_xs[mask]]).permute((1, 0)).long()

        return [batch_ids, center_zyxs, topk_score[mask]]


def get_3Dcoord_maps(size=128, z_base=None):
    range_arr = ops.arange(size, dtype=mindspore.float32)
    if z_base is None:
        Z_map = range_arr.reshape((1, size, 1, 1, 1)).tile((1, 1, size, size, 1)) / size * 2 - 1
    else:
        Z_map = z_base.reshape((1, size, 1, 1, 1)).tile((1, 1, size, size, 1))
    Y_map = range_arr.reshape((1, 1, size, 1, 1)).tile((1, size, 1, size, 1)) / size * 2 - 1
    X_map = range_arr.reshape((1, 1, 1, size, 1)).tile((1, size, size, 1, 1)) / size * 2 - 1
...
def gather_feature(fmap, index, mask=None, use_transform=False):
    if use_transform:
        ...
        fmap = fmap.view(batch, channel, -1).permute((0, 2, 1))
...
def gaussian3D(d, h, w, center, s=2, device='cuda'):
    ...
    """
    x = ops.arange(0, w, 1).float()
    y = ops.arange(0, h, 1).float()
    y = y.unsqueeze(1)
    z = ops.arange(0, d, 1).float()
    ...
    return ops.exp(-1 * ((x - x0) ** 2 + (y - y0) ** 2 + (z - z0) ** 2) / s ** 2)


def process_center(center_gt, centermap):
    center_list = []
    center_locs = ops.stack(ops.where(centermap[0] > 0.25))
    center_locs = ops.swapaxes(center_locs, axis0=1, axis1=0)
    dists = []
    for center in center_gt:
        dists.append(ops.norm(center_locs.float() - center[None].float(), axis=1))
    dists = ops.stack(dists)
    assign_id = ops.argmin(dists, 0)
...
def test_centermaps():
    ...
    for i in range(batch_size):
        centermaps.append(mindspore.Tensor.from_numpy(CM.generate_centermap(center_locs, bboxes_hw_norm=bboxes)))
    centermaps = ops.stack(centermaps)
...
def visualize_3d_hmap(hmap):
    # type: (Union[np.ndarray, mindspore.Tensor]) -> None
    """
...
def test_centermaps_3D(visualize=False):
    ...
    center_locs = mindspore.Tensor.from_numpy(center_locs)
