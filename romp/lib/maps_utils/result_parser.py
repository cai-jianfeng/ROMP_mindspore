...
class ResultParser(nn.Cell):
    def __init__(self, with_smpl_parser=True):
        super(ResultParser, self).__init__()
        self.map_size = args().centermap_size  # 64
        self.with_smpl_parser = with_smpl_parser
        if args().calc_smpl_mesh and with_smpl_parser:  # 不会进入
    ...
    def suppressing_silimar_mesh_and_2D_center(self, params_preds, pred_batch_ids, pred_czyxs, top_score,
                                               center2D_thresh=5,
                                               pose_thresh=2.5):  # center2D_thresh=5, pose_thresh=2.5 center2D_thresh=2, pose_thresh=1.2
    ...
    def match_params_new(self, outputs, meta_data, cfg):
        gt_keys = ['params', 'full_kp2d', 'kp_3d', 'valid_masks', 'subject_ids', 'verts', 'cam_mask',
                   'kid_shape_offsets', 'root_trans', 'cams']
        if args().learn_relative:
            gt_keys += ['depth_info']

        exclude_keys = ['heatmap', 'centermap', 'AE_joints', 'person_centers', 'params_pred',
                        'all_person_detected_mask', "person_scales"]
    ...
    def match_gt_pred_3d_2d(self, center_gts_info_2d, center_gts_info_3d, pred_batch_ids, pred_czyxs, top_score,
                            cam_czyx, device, is_training, batch_size=1, with_2d_matching=True):
...
def _check_params_sampling_(param_maps_shape, dim_start, dim_end, batch_ids, sampler_flat_inds_i):
    ...
    assert param_maps_shape[2] > dim_end >= dim_start, \
        logging.error(
            'During parameter sampling, param_maps dimension -1 is not larger than dim_end and dim_start, they are {},{},{}'.format(
                param_maps_shape[-1], dim_end, dim_start))
    assert (batch_ids >= param_maps_shape[0]).sum() == 0, \
        logging.error(
            'During parameter sampling, batch_ids {} out of boundary, param_maps_shape[0] is {}'.format(batch_ids,
                                                                                                        param_maps_shape[
                                                                                                            0]))
    assert (sampler_flat_inds_i >= param_maps_shape[1]).sum() == 0, \
        logging.error(
            'During parameter sampling, sampler_flat_inds_i {} out of boundary, param_maps_shape[1] is {}'.format(
