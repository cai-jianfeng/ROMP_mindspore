...
class SMPLA_parser(nn.Cell):
    ...
    def construct(self, betas=None, poses=None, root_align=True):
        baby_mask = betas[:,10] > self.baby_thresh
        if baby_mask.sum()>0:
            adult_mask = ~baby_mask
            verts, joints = ops.zeros(len(poses), 6890, 3, device=poses.device, dtype=poses.dtype), ops.zeros(len(poses), 54+17, 3, device=poses.device, dtype=poses.dtype)
