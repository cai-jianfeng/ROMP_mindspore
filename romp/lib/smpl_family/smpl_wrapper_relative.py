...
class SMPLWrapper(nn.Cell):
    def __init__(self):
        ...
        self.params_num = mindspore.Tensor(self.part_idx).sum().numpy().item()
        # print(type(self.params_num))
        self.global_orient_nocam = mindspore.Tensor.from_numpy(constants.global_orient_nocam).unsqueeze(0)
        self.joint_mapper_op25 = mindspore.Tensor.from_numpy(constants.joint_mapping(constants.SMPL_ALL_54, constants.OpenPose_25)).long()
    ...
    def pack_params_dict(self, params_pred):
        ...
        params_dict['body_pose'] = ops.cat([params_dict['body_pose'], ops.zeros(
            N, 6).to(params_dict['body_pose'].device)], 1)
        params_dict['poses'] = ops.cat(
...
def parse_age_cls_results(age_probs):
    age_preds = ops.ones_like(age_probs).long()*-1
...
def merge_smpl_outputs(results_list):
    ...
    map_inds = ops.cat([ops.where(mask)[0] for _,mask in results_list], 0)
    for key in list(results.keys()):
        results[key] = ops.cat([result[key] for result,_ in results_list], 0)[map_inds]
    return results