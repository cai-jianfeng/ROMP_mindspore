...
class COCOeval:
    ...
    def computeOks(self, imgId, catId):
        ...
        for j, gt in enumerate(gts):
            ...
            g = mindspore.Tensor(gt['keypoints'])
            xg = g[0::3]
            yg = g[1::3]
            vg = g[2::3]
            ...
            for i, dt in enumerate(dts):
                d = mindspore.Tensor(dt['keypoints'])
                xd = d[0::3]
                yd = d[1::3]
                if k1 > 0:
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        gtm = np.zeros((T, G))
        dtm = np.zeros((T, D))
        gtIg = mindspore.Tensor([g['_ignore'] for g in gt])
        dtIg = np.zeros((T, D))
        if not len(ious) == 0:
        ...
        a = mindspore.Tensor([d['area'] < aRng[0] or d['area'] > aRng[1]
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                        tp = mindspore.Tensor(tp)
                        fp = mindspore.Tensor(fp)
                        nd = len(tp)
                        rc = tp / npig
                        ...
                        try:
                            ...
                            pass
                        precision[t, :, k, a, m] = mindspore.Tensor(q)
                        scores[t, :, k, a, m] = mindspore.Tensor(ss)
        self.eval = {
            'params': p,
            'counts': [T, R, K, A, M],
