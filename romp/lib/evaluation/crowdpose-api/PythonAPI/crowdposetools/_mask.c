 ...
 * 
 *     # called when passing into mindspore.Tensor() and return an np.ndarray in column-major order
 *     def __array__(self):             # <<<<<<<<<<<<<<
 ...
 * def encode(np.ndarray[np.uint8_t, naxis=3, mode='fortran'] mask):             # <<<<<<<<<<<<<<
 ...
 *     cdef np.npy_intp shape[1]
 *     shape[0] = <np.npy_intp> Rs._n             # <<<<<<<<<<<<<<
 *     a = mindspore.Tensor((Rs._n, ), dtype=np.uint8)
 ...
 *         elif type(objs) == list:             # <<<<<<<<<<<<<<
 *             # check if list is in box format and convert it to np.ndarray
 *             isbox = np.all(mindspore.Tensor([(len(obj)==4) and ((type(obj)==list) or (type(obj)==np.ndarray)) for obj in objs]))
 */
 ...
 *             # check if list is in box format and convert it to np.ndarray
 *             isbox = np.all(mindspore.Tensor([(len(obj)==4) and ((type(obj)==list) or (type(obj)==np.ndarray)) for obj in objs]))             # <<<<<<<<<<<<<<
 *             isrle = np.all(mindspore.Tensor([type(obj) == dict for obj in objs]))
 *             if isbox:
 */
 ...
 *             isrle = np.all(mindspore.Tensor([type(obj) == dict for obj in objs]))             # <<<<<<<<<<<<<<
 *             if isbox:
 *                 objs = mindspore.Tensor(objs, dtype=np.double)
 */
 ...
 *             raise Exception('unrecognized type.  The following type: RLEs (rle), np.ndarray (box), and list (box) are supported.')             # <<<<<<<<<<<<<<
 *         return objs
 *     def _rleIou(RLEs dt, RLEs gt, np.ndarray[np.uint8_t, naxis=1] iscrowd, siz m, siz n, np.ndarray[np.double_t,  naxis=1] _iou):
 */
  /*else*/ {
 ...
 *     def _rleIou(RLEs dt, RLEs gt, np.ndarray[np.uint8_t, naxis=1] iscrowd, siz m, siz n, np.ndarray[np.double_t,  naxis=1] _iou):             # <<<<<<<<<<<<<<
 *         rleIou( <RLE*> dt._R, <RLE*> gt._R, m, n, <byte*> iscrowd.data, <double*> _iou.data )
 *     def _bbIou(np.ndarray[np.double_t, naxis=2] dt, np.ndarray[np.double_t, naxis=2] gt, np.ndarray[np.uint8_t, naxis=1] iscrowd, siz m, siz n, np.ndarray[np.double_t, naxis=1] _iou):
 */
 ...
 *         return N             # <<<<<<<<<<<<<<
 *     # convert iscrowd to numpy array
 *     cdef np.ndarray[np.uint8_t, naxis=1] iscrowd = mindspore.Tensor(pyiscrowd, dtype=np.uint8)
 */
  __Pyx_XDECREF(__pyx_r);
 ...
 *     cdef np.npy_intp shape[1]
 *     shape[0] = <np.npy_intp> 4*n             # <<<<<<<<<<<<<<
 *     bb = mindspore.Tensor((1,4*n), dtype=np.double)
 ...
 *     return bb             # <<<<<<<<<<<<<<
 * 
 * def frBbox(np.ndarray[np.double_t, naxis=2] bb, siz h, siz w ):
 */
  __Pyx_XDECREF(__pyx_r);
 ...
 * 
 * def frPoly( poly, siz h, siz w ):             # <<<<<<<<<<<<<<
 *     cdef np.ndarray[np.double_t, naxis=1] np_poly
 *     n = len(poly)
 */
 ...
 *     Rs = RLEs(n)             # <<<<<<<<<<<<<<
 *     for i, p in enumerate(poly):
 *         np_poly = mindspore.Tensor(p, dtype=np.double, order='F')
 */
 ...
 * 
 * def frUncompressedRLE(ucRles, siz h, siz w):             # <<<<<<<<<<<<<<
 *     cdef np.ndarray[np.uint32_t, naxis=1] cnts
 *     cdef RLE R
 */
 ...
 *     for i in range(n):             # <<<<<<<<<<<<<<
 *         Rs = RLEs(1)
 *         cnts = mindspore.Tensor(ucRles[i]['counts'], dtype=np.uint32)
 */
  __pyx_t_1 = __pyx_v_n;
  __pyx_t_3 = __pyx_t_1;
  for (__pyx_t_4 = 0; __pyx_t_4 < __pyx_t_3; __pyx_t_4+=1) {
