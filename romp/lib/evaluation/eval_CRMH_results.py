...
class Evaluate(Base):
    ...
    def test_3doh50k(self):
        ...
        for img_name, annot in self.annotations.items():
            ...
            person_id = np.argmax(mindspore.Tensor(confidences))
            kp3d_preds = ops.einsum('bik,ji->bjk', [pred_vertices, J_regressor])[person_id]
            kp3d_gt = mindspore.Tensor.from_numpy(mindspore.Tensor(annot['smpl_joints_3d'])/10.)
            
            real_3d.append(kp3d_gt)
            predicts.append(kp3d_preds)

        real_3d, predicts = ops.stack(real_3d), ops.stack(predicts)
    ...
    def load_gt(self):
        ...
        if self.dataset_eval_crmh == '3DOH50K':
            ...
            for action_name, clip_num in PW3D_HCsubset.items():
                ...
                poses2d_gt = mindspore.Tensor(poses2d_gt).transpose(0,1,3,2)
        ...
        with ops.stop_gradient():
            for test_iter,data_3d in enumerate(self.data_loader):
                for batch_id, imgpath in enumerate(data_3d['imgpath']):
                    ...
                    kp3d_matched = mindspore.Tensor(kp3d_matched)
                    gt_valid_idx = mindspore.Tensor(gt_valid_idx)
                    predicts.append(mindspore.Tensor.from_numpy(kp3d_matched).float())
                    real_3d.append(annot_gt[gt_valid_idx])

        real_3d, predicts = ops.cat(real_3d,0), ops.cat(predicts,0)
        abs_error = self.calc_mpjpe(real_3d, predicts, lrhip=self.lr_hip_idx_smpl24).float().cpu().numpy()*1000
        rt_error = self.calc_pampjpe(real_3d, predicts, lrhip=self.lr_hip_idx_smpl24).float().cpu().numpy()*1000
