...
class SMPL(object):
    def __init__(self, center_idx=None,  gender='neutral', model_root = '/models'):
        ...
        self.shapedirs = mindspore.Tensor(smpl_data['shapedirs'])

        self.posedirs = mindspore.Tensor(smpl_data['posedirs'])

        self.v_template = mindspore.Tensor(smpl_data['v_template'])
        self.v_template = np.expand_dims(self.v_template, axis = 0)

        self.J_regressor = mindspore.Tensor(smpl_data['J_regressor'].toarray())

        self.weights = mindspore.Tensor(smpl_data['weights'])

        self.faces =mindspore.Tensor(smpl_data['f'].astype(np.int32))
