...
SMPL_OR_JOINTS = mindspore.Tensor([0, 1, 2, 4, 5, 16, 17, 18, 19])
...
def joint_angle_error(pred_mat, gt_mat):
    ...
    return np.mean(mindspore.Tensor(angles))
...
def compute_errors(preds3d, gt3ds):
    ...
    return np.mean(mindspore.Tensor(errors)), np.mean(mindspore.Tensor(errors_pa)), \
    ...
    for path_pred, path_gt in zip(paths_pred, paths_gt):
        ...
        for i in range(len(genders)):
            ...
            if 'jointPositions' in keys_pred:
                ...
                jp_pred = mindspore.Tensor(data_pred['jointPositions'])
            ...
            if 'orientations' in keys_pred:
                ...
                glob_rot_pred = mindspore.Tensor(data_pred['orientations'])
                # select a subset of prediction data
            ...
            if 'smpl_params' in keys_pred:
                ...
                params_pred = mindspore.Tensor(data_pred['smpl_params'])
