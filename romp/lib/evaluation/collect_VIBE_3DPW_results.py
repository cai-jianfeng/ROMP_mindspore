...
class Submit(object):
    def __init__(self):
        ...
        self.joint_regressor = mindspore.Tensor.from_numpy(csr_matrix.toarray(self.read_pickle(os.path.join(self.project_dir,'models/smpl_original/basicModel_f_lbs_10_207_0_v1.0.0.pkl'))['J_regressor'])).float().T
    ...
    def eval_pve(self, submit_dir, params_results):
        ...
        params_preds[:,:3] = 0
        params_gts[:,:3] = 0
        PVE = np.mean(compute_error_verts(target_theta=mindspore.Tensor.from_numpy(params_gts).float(), pred_theta=mindspore.Tensor.from_numpy(params_preds).float())) * 1000
        print('PVE: ', PVE)
            ...
            if os.path.exists(result_path):
                ...
                for subject_id in range(subject_num):
                    ...
                    for frame_id in pred_results:
                        ...
                        closet_frame_dist = np.min(mindspore.Tensor(list(frame_dist_dict.keys())))
                    ...
                    self.results[action_name][subject_id] = [mindspore.Tensor(pose_results), mindspore.Tensor(kp3d_results), mindspore.Tensor(bbox_results), mindspore.Tensor(frame_ids), shape_results,verts_results]
            else:
    ...
    def re_tracking(self, data, first_frame_bbox_gt):
        ...
        closet_first_frame_dist = np.min(mindspore.Tensor(list(first_frame_dist.keys())))
        ...
        frame_ids_retracked.append(mindspore.Tensor(result[closet_subject_id_first_frame]['frame_ids']))
        ...
        while frame_ids_retracked[-1][-1]<frame_num-1 and len(person_id_list)>0:
            ...
            closet_frame_dist = np.min(mindspore.Tensor(list(frame_dist.keys())))
            ...
            frame_ids_retracked.append(mindspore.Tensor(result[closet_subject_id]['frame_ids']))
    ...
    def match_tracking(self, data):
        ...
        target_ids = np.argsort(mindspore.Tensor([len(result[person_id]['frame_ids']) for person_id in person_id_list]))[::-1][:subject_num]
        ...
        while len(target_ids)<subject_num:
            ...
            target_ids = np.concatenate([target_ids,mindspore.Tensor([target_ids[-1]])])

        person_id_sellected = mindspore.Tensor(person_id_list)[target_ids]
        ...
        person_ids_matched = mindspore.Tensor(person_ids_matched).astype(np.uint8)
    ...
    def regress_kp3d_from_verts(self,verts):
        verts = mindspore.Tensor.from_numpy(verts)
        joint_x = ops.matmul(verts[:, :, 0], self.joint_regressor)
        joint_y = ops.matmul(verts[:, :, 1], self.joint_regressor)
        joint_z = ops.matmul(verts[:, :, 2], self.joint_regressor)

        joints = ops.stack([joint_x, joint_y, joint_z], dim = 2)
        return joints
    ...
    def pack_results(self,save_dir):
        ...
        for action_id,action_name in enumerate(self.results):
            for subject_id,[pose_preds, kp3d_smpl, bboxes,frame_ids,shape_results,verts_results] in self.results[action_name].items():
                ...
                #print(action_name,subject_id,mindspore.Tensor(frame_ids))
                params_results[action_name][0][subject_id][mindspore.Tensor(frame_ids)] = pose_preds.copy()
                params_results[action_name][1][subject_id][mindspore.Tensor(frame_ids)] = shape_results
                pose_result = []
                for pose_pred in pose_preds:
                    params_processed = self.process_params(mindspore.Tensor.from_numpy(pose_pred))
                    pose_result.append(params_processed)
                results[split][action_name][0][subject_id][mindspore.Tensor(frame_ids)] = kp3d_smpl
                results[split][action_name][1][subject_id][mindspore.Tensor(frame_ids)] = mindspore.Tensor(pose_result)
                bbox_results[action_name][subject_id][mindspore.Tensor(frame_ids)] = mindspore.Tensor(bboxes)
    ...
    def process_params(self,params):
        ...
        rotation_final = mindspore.Tensor(rotation_final)
...
def check_valid_inds(poses2d, camposes_valid):
    ...
    camposes_valid = mindspore.Tensor(camposes_valid).astype('bool')

    final = np.logical_and(poses2d_bool_inv, camposes_valid)
    indices = mindspore.Tensor(np.where(final == True)[0])
...
def get_data(params_results, paths_gt):
    ...
    for path_gt in paths_gt:
        ...
        for i in range(len(genders)):
            ...
            pose_params = mindspore.Tensor(data_gt['poses'])
            pose_params = pose_params[i, valid_indices, :]

            shape_params = mindspore.Tensor(data_gt['betas'][i])
            ...
            #params_results[action_name][0][subject_id][mindspore.Tensor(frame_ids)] = pose_preds
            #params_results[action_name][1][subject_id][mindspore.Tensor(frame_ids)] = shape_results
