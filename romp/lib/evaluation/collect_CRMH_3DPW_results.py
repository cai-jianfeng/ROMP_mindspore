        ...
        if not os.path.exists(annot_file):
            ...
            for file_name, result in outputs_dict.items():
                ...
                if result is not None:
                    ...
                    result['bbox'] = mindspore.Tensor(bbox_detected_org)
                    #del result['verts']
    ...
    def pack_results(self,save_dir):
        ...
        for action_id,action_name in enumerate(self.results):
            for subject_id,[pose_preds, kp3d_smpl, bboxes,frame_ids,shape_results,verts_results] in self.results[action_name].items():
                ...
                frame_ids = mindspore.Tensor(frame_ids).astype(np.int)
                ...
                results[split][action_name][1][subject_id][frame_ids] = mindspore.Tensor(pose_result)
...
def calc_center(bbox):
    ...
    bbox_center = mindspore.Tensor([(tl_x+rb_x)/2.,(tl_y+rb_y)/2.])
    return bbox_center
...
def convert_to_vect(pose_preds):
    ...
    converted = mindspore.Tensor(converted).reshape((-1,72))
    return converted
