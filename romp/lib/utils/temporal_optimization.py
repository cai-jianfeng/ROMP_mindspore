...
def extract_motion_sequence(results_track_video, video_track_ids):
    ...
    subject_ids = np.unique(mindspore.Tensor(subject_ids))
    for subject_id in subject_ids:
        frame_ids_appear = {fid:np.where(mindspore.Tensor(track_ids)==subject_id)[0][0] for fid, track_ids in video_track_ids.items() if subject_id in track_ids}
...
def temporal_optimize_result(result, filter_dict):
  ...
  pose_euler = mindspore.Tensor([transform_rot_representation(vec, input_type='vec',out_type='euler') for vec in result['poses'].reshape((-1,3))])
  ...
  result['poses'][3:] = mindspore.Tensor([transform_rot_representation(bodypose, input_type='euler',out_type='vec') for bodypose in body_pose_euler.reshape(-1,3)]).reshape(-1)
  return result
