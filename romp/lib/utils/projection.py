...
def filter_out_incorrect_trans(kp_3ds, trans, kp_2ds, thresh=20, focal_length=args().focal_length,
                               center_offset=mindspore.Tensor([args().input_size, args().input_size]) / 2.):
    valid_mask = np.logical_and(kp_3ds[:, :, -1] != -2., kp_2ds[:, :, -1] > 0)
    projected_kp2ds = perspective_projection(kp_3ds, translation=trans, camera_center=center_offset,
                                             focal_length=focal_length, normalize=False)
    dists = (np.linalg.norm(projected_kp2ds.numpy() - kp_2ds, axis=-1, ord=2) * valid_mask).sum(-1) / (
                valid_mask.sum(-1) + 1e-3)
    cam_mask = dists < thresh
    assert len(trans) == len(cam_mask), print(
        'len(trans)==len(cam_mask) fail, trans {}; cam_mask {}'.format(trans, cam_mask))
...
def convert_kp2d_from_input_to_orgimg(kp2ds, offsets):
    ...
    if kp2ds.shape[-1] == 3:
        kp2ds_on_orgimg = ops.cat([kp2ds_on_orgimg, (kp2ds[:, :, [2]] + 1) * img_pad_size.unsqueeze(1)[:, :, [0]] / 2],
                                  -1)
...
def vertices_kp3d_projection(j3d_preds, cam_preds, joints_h36m17_preds=None, vertices=None, input2orgimg_offsets=None,
    ...
    if presp:
        ...
        pj3d = perspective_projection(j3d_preds, translation=pred_cam_t, focal_length=args().focal_length,
                                      normalize=True)
        projected_outputs = {'cam_trans': pred_cam_t, 'pj2d': pj3d[:, :, :2].float()}
        if joints_h36m17_preds is not None:
            pj3d_h36m17 = perspective_projection(joints_h36m17_preds, translation=pred_cam_t,
                                                 focal_length=args().focal_length, normalize=True)
            projected_outputs['pj2d_h36m17'] = pj3d_h36m17[:, :, :2].float()
        if vertices is not None:
            projected_outputs['verts_camed'] = perspective_projection(vertices.clone().detach(), translation=pred_cam_t,
                                                                      focal_length=args().focal_length, normalize=True,
                                                                      keep_axis=True)
...
def perspective_projection(points, translation=None, rotation=None, keep_dim=False,
    ...
    if ops.isnan(points).sum() > 0 or ops.isnan(projected_points).sum() > 0:
        print('translation:', translation[ops.where(ops.isnan(translation))[0]])
        print('points nan value number:', len(ops.where(ops.isnan(points))[0]))
        # print('projected_points nan value:', projected_points[ops.where(ops.isnan(projected_points))[0]])
        # print('projected_points nan', ops.where(ops.isnan(projected_points)))
