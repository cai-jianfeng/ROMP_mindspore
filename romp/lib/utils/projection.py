...
def filter_out_incorrect_trans(kp_3ds, trans, kp_2ds, thresh=20, focal_length=args().focal_length, center_offset=mindspore.Tensor([args().input_size, args().input_size])/2.):
...
def convert_kp2d_from_input_to_orgimg(kp2ds, offsets):
    ...
    if kp2ds.shape[-1] == 3:
        kp2ds_on_orgimg = ops.cat([kp2ds_on_orgimg, (kp2ds[:,:,[2]] + 1) * img_pad_size.unsqueeze(1)[:,:,[0]] / 2 ], -1)
...
def vertices_kp3d_projection(j3d_preds, cam_preds, joints_h36m17_preds=None, vertices=None, input2orgimg_offsets=None, presp=args().model_version>3):
    if presp:
        ...
        if vertices is not None:
            projected_outputs['verts_camed'] = perspective_projection(vertices.clone().detach(),translation=pred_cam_t,focal_length=args().focal_length, normalize=True, keep_axis=True)
        ...
        if vertices is not None:
            projected_outputs['verts_camed'] = batch_orth_proj(vertices, cam_preds, mode='3d',keep_axis=True)
