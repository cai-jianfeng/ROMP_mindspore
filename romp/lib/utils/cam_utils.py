...
cam3dmap_anchor = (constants.get_cam3dmap_anchor(args().FOV,args().centermap_size))#.float()
...
def convert_scale_to_depth_level(scale):
    ...
    return ops.argmin(ops.abs(scale[:,None].repeat(1, scale_num) - cam3dmap_anchors), axis=1)

def convert_cam_params_to_centermap_coords(cam_params):
    center_coords = ops.ones_like(cam_params)
    center_coords[:,1:] = cam_params[:,1:].clone()
    cam3dmap_anchors = cam3dmap_anchor.to(cam_params.device)[None]
    if len(cam_params) != 0:
        center_coords[:,0] = ops.argmin(ops.abs(cam_params[:,[0]].repeat(1, scale_num) - cam3dmap_anchors), axis=1).float()/args().centermap_size * 2. - 1.
...
def estimate_translation_cv2(joints_3d, joints_2d, focal_length=args().focal_length, img_size=mindspore.Tensor([512.,512.]), proj_mat=None, cam_dist=None):
    if proj_mat is None:
        camK = np.eye(3)
...
def estimate_translation_np(joints_3d, joints_2d, joints_conf, focal_length=args().focal_length, img_size=mindspore.Tensor([512.,512.]), proj_mat=None):
...
def estimate_translation(joints_3d, joints_2d, pts_mnum=4,focal_length=args().focal_length, proj_mats=None, cam_dists=None,img_size=mindspore.Tensor([512.,512.]), pnp_algorithm='cv2'):
    """Find camera translation that brings 3D joints joints_3d closest to 2D the corresponding joints_2d.
    Input:
