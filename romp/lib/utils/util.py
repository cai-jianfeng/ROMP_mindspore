...
def read_h5(name):
    ...
    for item, value in f.items():
        info[item] = mindspore.Tensor(value)
    f.close()
    return info
...
def BHWC_to_BCHW(x):
    ...
    """
    # x.unsqueeze(1) => B x 1 x H x W x C
    # transpose(1, -1): 交换 1 和 C
    return x.unsqueeze(1).transpose((0, 4, 2, 3, 1)).squeeze(-1)
...
def wrap(func, *args, unsqueeze=False):
    ...
    for i, arg in enumerate(args):
        if type(arg) == np.ndarray:
            args[i] = mindspore.Tensor.from_numpy(arg)
            if unsqueeze:
    ...
    if isinstance(result, tuple):
        ...
        for i, res in enumerate(result):
            if type(res) == mindspore.Tensor:
                if unsqueeze:
                    res = res.squeeze(0)
                result[i] = res.numpy()
        return tuple(result)
    elif type(result) == mindspore.Tensor:
        if unsqueeze:
...
def write_words2img(img, height_use, words, line_length=20, line_height=36, width_min=1420, color=(0, 0, 0),
                    duan_space=True):
...
def calc_aabb_batch(ptSets_batch):
    ...
    ptLeftTop = mindspore.Tensor([np.min(ptSets_batch[:, :, 0], axis=1), np.min(ptSets_batch[:, :, 1], axis=1)]).T
    ptRightBottom = mindspore.Tensor([np.max(ptSets_batch[:, :, 0], axis=1), np.max(ptSets_batch[:, :, 1], axis=1)]).T
...
def batch_rodrigues(param):
    ...
    l1norm = ops.norm(param + 1e-8, p=2, dim=1)
    angle = ops.unsqueeze(l1norm, -1)
    normalized = ops.div(param, angle)
    angle = angle * 0.5
    # 上面算出的是一个向量的长度：sqrt(x**2+y**2+z**2)/2,所以这个长度的的cos
    v_cos = ops.cos(angle)
    v_sin = ops.sin(angle)
    # 用四元组表示三维旋转，有时间看一下×××××××××
    quat = ops.cat([v_cos, v_sin * normalized], dim=1)
...
def quat2mat(quat):
    ...
    rotMat = ops.stack([w2 + x2 - y2 - z2, 2 * xy - 2 * wz, 2 * wy + 2 * xz,
                        2 * wz + 2 * xy, w2 - x2 + y2 - z2, 2 * yz - 2 * wx,
                        2 * xz - 2 * wy, 2 * wx + 2 * yz, w2 - x2 - y2 + z2], axis=1).view(B, 3, 3)
    return rotMat
...
def batch_global_rigid_transformation(Rs, Js, parent, rotate_base=False, root_rot_mat=None):
    ...
    # 确定根节点的旋转变换。
    if rotate_base:
        np_rot_x = mindspore.Tensor([[1, 0, 0], [0, -1, 0], [0, 0, -1]], dtype=np.float)
        np_rot_x = np.reshape(np.tile(np_rot_x, [N, 1]), [N, 3, 3])
        rot_x = mindspore.Tensor.from_numpy(np_rot_x).float().cuda()
        root_rotation = ops.matmul(Rs[:, 0, :, :], rot_x)
        ...
        root_rotation = Rs[:, 0, :, :]
    Js = ops.unsqueeze(Js, -1)

    def make_A(R, t):
        R_homo = ops.pad(R, [0, 0, 0, 1, 0, 0])
        t_homo = ops.cat([t, ops.ones(N, 1, 1).cuda()], dim=1)
        return ops.cat([R_homo, t_homo], 2)
    ...
    for i in range(1, parent.shape[0]):
        ...
        res_here = ops.matmul(results[parent[i]], A_here)
        results.append(res_here)

    results = ops.stack(results, dim=1)

    new_J = results[:, :, :3, 3]
    # print('result',results)
    Js_w0 = ops.cat([Js, ops.zeros(N, 24, 1, 1).cuda()], dim=2)
    # print('js w ',Js_w0)
    init_bone = ops.matmul(results, Js_w0)
    # print('init_bone before padded',init_bone)
    init_bone = ops.pad(init_bone, [3, 0, 0, 0, 0, 0, 0, 0])
...
def batch_global_rigid_transformation_cpu(Rs, Js, parent, rotate_base=False, root_rot_mat=None):
    ...
    if rotate_base:
        np_rot_x = mindspore.Tensor([[1, 0, 0], [0, -1, 0], [0, 0, -1]], dtype=np.float)
        np_rot_x = np.reshape(np.tile(np_rot_x, [N, 1]), [N, 3, 3])
        rot_x = mindspore.Tensor.from_numpy(np_rot_x).float()
        root_rotation = ops.matmul(Rs[:, 0, :, :], rot_x)
    elif root_rot_mat is not None:
        np_rot_x = np.reshape(np.tile(root_rot_mat, [N, 1]), [N, 3, 3])
        rot_x = mindspore.Tensor.from_numpy(np_rot_x).float()
    ...
    def make_A(R, t):
        R_homo = ops.pad(R, [0, 0, 0, 1, 0, 0])
        t_homo = ops.cat([t, ops.ones(N, 1, 1)], dim=1)
        return ops.cat([R_homo, t_homo], 2)
    ...
    new_J = results[:, :, :3, 3]
    Js_w0 = ops.cat([Js, ops.zeros(N, 24, 1, 1)], dim=2)
    init_bone = ops.matmul(results, Js_w0)
    init_bone = ops.pad(init_bone, [3, 0, 0, 0, 0, 0, 0, 0])
    A = results - init_bone
...
def rotation_matrix_to_angle_axis(rotation_matrix):
    ...
    if rotation_matrix.shape[1:] == (3, 3):
        ...
        hom = mindspore.Tensor([0, 0, 1], dtype=mindspore.float32,
                               device=rotation_matrix.device).reshape(1, 3, 1).expand(rot_mat.shape[0], -1, -1)
        rotation_matrix = ops.cat([rot_mat, hom], axis=-1)
