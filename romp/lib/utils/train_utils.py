...
def load_model(path, model, prefix='module.', drop_prefix='', optimizer=None, **kwargs):
    ...
    if os.path.exists(path):
        pretrained_model = mindspore.load_checkpoint(path)
        current_model = model.parameters_dict()
...
#     used_idx_now = mindspore.Tensor(used_idx_now)
...
def fix_backbone(params, exclude_key=['backbone.']):
    for exclude_name in exclude_key:
        for index, (name, param) in enumerate(params.get_parameters()):
...
def train_entire_model(net):
    exclude_layer = []
    # mindspore 的 nn.Cell 类对象的 get_parameters 放回迭代器
    for index, (param) in enumerate(net.get_parameters()):
        if 'smpl' not in param.name:
            param.requires_grad = True
        else:
            if param.requires_grad:
                exclude_layer.append(param.name)
            param.requires_grad = False
