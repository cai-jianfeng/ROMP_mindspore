...
def model_info(model):  # Plots a line-by-line description of a PyTorch model
    ...
    for i, (name, p) in enumerate(model.get_parameters()):
...
def xyxy2xywh(x):
    ...
    y = ops.zeros(x.shape) if x.dtype is mindspore.float32 else np.zeros(x.shape)
    y[:, 0] = (x[:, 0] + x[:, 2]) / 2
    y[:, 1] = (x[:, 1] + x[:, 3]) / 2
    y[:, 2] = x[:, 2] - x[:, 0]
    y[:, 3] = x[:, 3] - x[:, 1]
...
def scale_coords(img_size, coords, img0_shape):
    ...
    coords[:, [1, 3]] -= pad_y
    coords[:, 0:4] /= gain
    coords[:, :4] = ops.clamp(coords[:, :4], min=0)
    return coords
...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    tp, conf, pred_cls, target_cls = mindspore.Tensor(tp), mindspore.Tensor(conf), mindspore.Tensor(pred_cls), mindspore.Tensor(target_cls)
    ...
    return mindspore.Tensor(ap), unique_classes.astype('int32'), mindspore.Tensor(r), mindspore.Tensor(p)
...
def bbox_iou(box1, box2, x1y1x2y2=False):
    ...
    inter_rect_x1 = ops.max(b1_x1.unsqueeze(1), b2_x1)
    inter_rect_y1 = ops.max(b1_y1.unsqueeze(1), b2_y1)
    inter_rect_x2 = ops.min(b1_x2.unsqueeze(1), b2_x2)
    inter_rect_y2 = ops.min(b1_y2.unsqueeze(1), b2_y2)
    # Intersection area
    inter_area = ops.clamp(inter_rect_x2 - inter_rect_x1, 0) * ops.clamp(inter_rect_y2 - inter_rect_y1, 0)
    # Union Area
...
def build_targets_max(target, anchor_wh, nA, nC, nGh, nGw):
    ...
    txy = ops.zeros(nB, nA, nGh, nGw, 2).cuda()  # batch size, anchors, grid size
    twh = ops.zeros(nB, nA, nGh, nGw, 2).cuda()
    ...
    for b in range(nB):
        ...
        gwh[:, 0] = gwh[:, 0] * nGw
        gwh[:, 1] = gwh[:, 1] * nGh
        gi = ops.clamp(gxy[:, 0], min=0, max=nGw -1).long()
        gj = ops.clamp(gxy[:, 1], min=0, max=nGh -1).long()

        # Get grid box indices and prevent overflows (i.e. 13.01 on 13 anchors)
        #gi, gj = ops.clamp(gxy.long(), min=0, max=nG - 1).t()
        #gi, gj = gxy.long().t()

        # iou of targets-anchors (using wh only)
        box1 = gwh
        box2 = anchor_wh.unsqueeze(1)
        inter_area = ops.min(box1, box2).prod(2)
        ...
        if nTb > 1:
            ...
            u = ops.stack((gi, gj, a), 0)[:, iou_order]
            # _, first_unique = np.unique(u, axis=1, return_index=True)  # first unique indices
            first_unique = return_torch_unique_index(u, ops.unique(u, axis=1))  # torch alternative
        ...
        # Width and height
        twh[b, a, gj, gi] = ops.log(gwh / anchor_wh[a])  # yolo method
        # twh[b, a, gj, gi] = ops.sqrt(gwh / anchor_wh[a]) / 2 # power method

        # One-hot encoding of label
        tcls[b, a, gj, gi, tc] = 1
        tconf[b, a, gj, gi] = 1
        tid[b, a, gj, gi] = t_id.unsqueeze(1)
    tbox = ops.cat([txy, twh], -1)
...
def generate_anchor(nGh, nGw, anchor_wh):
    nA = len(anchor_wh)
    yy, xx =torch.meshgrid(ms_np.arange(nGh), ms_np.arange(nGw))
    xx, yy = xx.cuda(), yy.cuda()

    mesh = ops.stack([xx, yy], axis=0)                                              # Shape 2, nGh, nGw
    ...
    anchor_mesh = ops.cat([mesh, anchor_offset_mesh], axis=1)                       # Shape nA x 4 x nGh x nGw
    return anchor_mesh
...
def encode_delta(gt_box_list, fg_anchor_list):
    ...
    dw = ops.log(gw/pw)
    dh = ops.log(gh/ph)
    return ops.stack([dx, dy, dw, dh], axis=1)
...
def decode_delta(delta, fg_anchor_list):
    ...
    gw = pw * ops.exp(dw)
    gh = ph * ops.exp(dh)
    return ops.stack([gx, gy, gw, gh], axis=1)
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.2):
    ...
    for image_i, pred in enumerate(prediction):
        ...
        if len(det_max) > 0:
            ...
            output[image_i] = det_max if output[image_i] is None else ops.cat((output[image_i], det_max))
...
def return_torch_unique_index(u, uv):
    ...
    first_unique = ops.zeros(n, device=u.device).long()
    for j in range(n):
...
def strip_optimizer_from_checkpoint(filename='weights/best.pt'):
    ...
    a = mindspore.load_checkpoint(filename, map_location='cpu')
    a['optimizer'] = []
    mindspore.save_checkpoint(a, filename.replace('.pt', '_lite.pt'))
