...
class Learnable_Loss(nn.Cell):
    ...
    def __init__(self, ID_num=0):
        ...
        self.loss_class = {'det': ['CenterMap', 'CenterMap_3D'],
                           'reg': ['MPJPE', 'PAMPJPE', 'P_KP2D', 'Pose', 'Shape', 'Cam', 'Prior']}
    ...
    def construct(self, outputs, new_training=False):
        ...
        for key, value in loss_dict.items():
            if isinstance(value, mindspore.Tensor):
                if not ops.isnan(value):
                    if value.numpy().item() < args().loss_thresh:
