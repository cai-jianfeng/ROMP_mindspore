...
def batch_kp_2d_l2_loss(real, pred):
    ...
    for parent_joint, leaf_joints in constants.joint2D_tree.items():
        ...
        leaf_ids = mindspore.Tensor([constants.SMPL_ALL_54[leaf_joint] for leaf_joint in leaf_joints])
        vis_mask[:, leaf_ids] = vis_mask[:, [parent_id]] * vis_mask[:, leaf_ids]
        real[:, leaf_ids] -= real[:, [parent_id]]
        pred[:, leaf_ids] -= pred[:, [parent_id]]
    bv_mask = ops.logical_and(vis_mask.sum(-1) > 0, (real - pred).sum(-1).sum(-1) != 0)
    ...
    if vis_mask.sum() > 0:
        ...
        if ops.isnan(loss).sum() > 0 or (loss > 1000).sum() > 0:
            return 0
