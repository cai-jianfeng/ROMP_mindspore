...
def t_t2m(tensor_t):
    ...
    '''
    tensor_n = tensor_t.numpy()
    return mindspore.Tensor(tensor_n)


class SMPLifyAnglePrior(nn.Cell):
    def __init__(self, dtype=mindspore.float32, **kwargs):
        ...
        angle_prior_idxs = mindspore.Tensor([55, 58, 12, 15], dtype=np.int64)
        angle_prior_idxs = mindspore.Tensor(angle_prior_idxs, dtype=mindspore.long)
...
class L2Prior(nn.Cell):
    ...
    def construct(self, module_input, *args):
        return ops.sum(module_input.pow(2))


class MaxMixturePrior(nn.Cell):
    ...
    def __init__(self, smpl_prior_path=args().smpl_prior_path,
        ...
        if dtype == DEFAULT_DTYPE:
            np_dtype = mindspore.float32
        elif dtype == mindspore.float64:
            np_dtype = np.float64
        else:
        ...
        self.means = mindspore.Parameter(mindspore.Tensor(means, dtype=dtype), 'means', requires_grad=False)
    ...
    def get_mean(self):
        ...
        mean_pose = ops.matmul(self.weights, self.means)
        return mean_pose
    ...
    def merged_log_likelihood(self, pose):
        ...
        #  curr_loglikelihood = 0.5 * (self.cov_dets.unsqueeze(axis=0) +
        #  self.random_var_dim * self.pi_term +
        #  diff_prec_quadratic
        #  ) - ops.log(self.weights)

        min_likelihood, _ = ops.min(curr_loglikelihood, axis=1)
    ...
    def log_likelihood(self, pose, *args, **kwargs):
        ...
        log_likelihoods = ops.stack(likelihoods, axis=1)
        min_idx = ops.argmin(log_likelihoods, axis=1)
        weight_component = self.nll_weights[:, min_idx]
        weight_component = -ops.log(weight_component)

        return weight_component + log_likelihoods[:, min_idx]

    def construct(self, pose):
        if self.use_merged:
...
class MultiLossFactory(nn.Cell):
    ...
    def construct(self, outputs, heatmaps, masks, joints):
        # forward check
