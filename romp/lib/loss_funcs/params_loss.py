...
def batch_l2_loss(real,predict):
    loss_batch = ops.norm(real-predict, p=2, axis=1)
...
def batch_smpl_pose_l2_error(real,predict):
    ...
    loss = ops.norm((real-predict).view(-1,9), p=2, axis=-1)#self.sl1loss(real,predict)#
...
def trans_relative_rot_to_global_rotmat(params, with_global_rot=False):
    ...
    if with_global_rot:
        sellect_joints = mindspore.Tensor([0,1,2,4,5,16,17,18,19],dtype=np.int)
        results = [pose_rotmat[:, 0]]
        for idx in range(param_num-1):
            i_val = int(idx + 1)
            joint_rot = pose_rotmat[:, i_val]
            parent = constants.kintree_parents[i_val]
            glob_transf_mat = ops.matmul(results[parent], joint_rot)
            results.append(glob_transf_mat)
    else:
        sellect_joints = mindspore.Tensor([1,2,4,5,16,17,18,19],dtype=np.int)-1
        results = [ops.eye(3,3)[None].cuda().repeat(batch_size,1,1)]
        for i_val in range(param_num-1):
            ...
            glob_transf_mat = ops.matmul(results[parent], joint_rot)
            results.append(glob_transf_mat)
    global_rotmat = ops.stack(results, axis=1)[:, sellect_joints].contiguous()
...
def _calc_joint_angle_error(pred_mat, gt_mat, return_axis_angle=False):
    ...
    angles = ops.norm(axis_angles, axis=-1)*(180./np.pi)
