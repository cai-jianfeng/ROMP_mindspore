...
def match_batch_subject_ids(reorganize_idx, subject_ids, torso_pj2d_errors, a_id, b_id, pj2d_thresh=0.1):
    ...
    for ind, sid in enumerate(all_subject_ids):
        a_ind = ms_np(ops.logical_and(subject_ids == sid, a_mask))[0][0]
        b_ind = ms_np(ops.logical_and(subject_ids == sid, b_mask))[0][0]
    ...
    matched_inds[0] = mindspore.Tensor(matched_inds[0]).long()
    matched_inds[1] = mindspore.Tensor(matched_inds[1]).long()
    return matched_inds
...
def relative_depth_loss(pred_depths, depth_ids, reorganize_idx, dist_thresh=0.3, uncertainty=None, matched_mask=None):
    ...
    for b_ind in ops.unique()(valid_inds):
        ...
        if did_num > 1:
            ...
            dist_mat = (pred_depths_sample.unsqueeze(0).tile(did_num, 1) - pred_depths_sample.unsqueeze(1).tile(1,did_num))[triu_mask]
            did_mat = (depth_ids[sample_inds].unsqueeze(0).tile(did_num, 1) - depth_ids[sample_inds].unsqueeze(1).tile(1,did_num))[triu_mask]
            sample_loss = []
            ...
            if args().depth_loss_type == 'Piecewise':
                ...
                if cd_mask.sum()>0:
                    cd_loss = ops.log(1+ops.exp(dist_mat[cd_mask]))
                    sample_loss.append(cd_loss)
                if fd_mask.sum()>0:
                    fd_loss = ops.log(1+ops.exp(-dist_mat[fd_mask]))
                    sample_loss.append(fd_loss)
            elif args().depth_loss_type == 'Log':
                eq_loss = dist_mat[did_mat==0]**2
                cd_loss = ops.log(1+ops.exp(dist_mat[did_mat<0]))
                fd_loss = ops.log(1+ops.exp(-dist_mat[did_mat>0]))
                sample_loss = [eq_loss, cd_loss, fd_loss]
            else:
                raise NotImplementedError
            
            if len(sample_loss)>0:
                this_sample_loss = ops.cat(sample_loss).mean()
...
def test_depth_ordering_loss():
    pred_cams = ops.rand(6,3)
    depth_info = ops.randint(5,(6,4))
    depth_info[1] = -1
    reorganize_idx = mindspore.Tensor([0,0,0,1,1,1])
...
def test_relative_shape_loss():
    pred_betas = ops.rand(4,13)
    depth_info = ops.randint(5,(4,4))
    depth_info[:,0] = ops.randint(4,(4,))
    depth_info[:,1] = ops.randint(1,(4,))
    depth_info[:,2] = ops.randint(2,(4,))
    depth_info[1] = -1
