...
def focal_loss(pred, gt):
    ...
    neg_weights = ops.pow(1 - gt, 4)

    loss = ops.zeros(gt.size(0)).to(pred.device)

    # when x <=0, log(x) lead to nan loss, collipsed
    # To take care of 1-pred_log in neg_loss, pred_log < 1-1e-4
    pred_log = ops.clamp(pred.clone(), min=1e-3, max=1-1e-3)
    pos_loss = ops.log(pred_log) * ops.pow(1 - pred, 2) * pos_inds
    neg_loss = ops.log(1 - pred_log) * ops.pow(pred, 2) * neg_weights * neg_inds
...
def make_input(t, requires_grad=False, need_cuda=True):
    inp = mindspore.Parameter(t, requires_grad=requires_grad)
    inp = inp.sum()
    if need_cuda:
        inp = inp.cuda()
    return inp
...
class HeatmapLoss(nn.Cell):
    ...
    def construct(self, pred, gt):
        ...
        if self.loss_type == 'focal':
            ...
            mask = gt.float().sum(axis=3).sum(axis=2).gt(0).float()
            loss = (((pred - gt)**2).mean(axis=3).mean(axis=2) * mask).sum()/mask.sum()
        else:
            raise NotImplementedError
        return loss


class AELoss(nn.Cell):
    ...
    def singleTagLoss(self, pred_tag, joints):
        ...
        for joints_per_person in joints:
            ...
            if len(tmp) == 0:
                continue
            tmp = ops.stack(tmp)
            tags.append(ops.mean(tmp, axis=0))
            pull = pull + ops.mean((tmp - tags[-1].expand_as(tmp))**2)

        num_tags = len(tags)
        if num_tags == 0:
            return make_input(ops.zeros(1).float()), \
                make_input(ops.zeros(1).float())
        elif num_tags == 1:
            return make_input(ops.zeros(1).float()), \
                pull/(num_tags)
        ...
        if self.loss_type == 'exp':
            diff = ops.pow(diff, 2)
            push = ops.exp(-diff)
            push = ops.sum(push) - num_tags
        elif self.loss_type == 'max':
            diff = 1 - ops.abs(diff)
            push = ops.clamp(diff, min=0).sum() - num_tags
        else:
    ...
    def construct(self, tags, joints):
        """
        ...
        for i in range(batch_size):
            ...
            pushes.append(push)
            pulls.append(pull)
        return ops.stack(pushes), ops.stack(pulls)
...
def test_ae_loss():
    ...
    joints[0, 1] = (22, 1)
    joints[1, 1] = (30, 1)
    joints = mindspore.Tensor(joints, dtype=mindspore.int64)
...
if __name__ == '__main__':
    ...
    x = ops.rand(2,num_joints,2).cuda()*2-1
    x[0,:2] = -2.
    heatmaps = hg.batch_process(x)
    print(heatmaps)
    loss = focal_loss(ops.sigmoid(heatmaps+ops.rand(1,25,128,128).cuda()),heatmaps)
    print(loss)
    #test_ae_loss()