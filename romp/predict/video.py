    ...
    def process_video(self, video_file_path):
        ...
        for test_iter,meta_data in enumerate(internet_loader):
            ...
            for frame_id in sorted(list(results_track.keys())):
                ...
                to_org_inds = mindspore.Tensor([np.where(reorganize_idx==ind)[0][0] for ind in reorganize_idx_uq])

                if self.show_largest_person_only:
                    max_id = np.argmax(mindspore.Tensor([result['cam'][0] for result in results_track[frame_id]]))
            ...
            if self.temporal_optimization or self.show_largest_person_only:
                ...
                for fid, frame_id in enumerate(sorted(list(results_track.keys()))):
                    for sid, tid in enumerate(video_track_ids[frame_id]):
                        ...
                        for item in item_names:
                            params_dict_new[item].append(mindspore.Tensor.from_numpy(results_track[frame_id][sid][item]))
                ...
                # update the vertices
                for item in item_names:
                    params_dict_new[item] = ops.stack(params_dict_new[item]).cuda()
                ...
                outputs['reorganize_idx'] = mindspore.Tensor(reorganize_idx_new)
