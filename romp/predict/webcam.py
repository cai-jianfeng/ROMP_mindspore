        ...
        while True:
            ...
            if outputs is not None and outputs['detection_flag']:
                ...
                if args().show_largest_person_only or self.visulize_platform == 'blender':
                    max_id = np.argmax(mindspore.Tensor([result['cam'][0] for result in results[frame_id]]))
                    results[frame_id] = [results[frame_id][max_id]]
                    tracked_ids = mindspore.Tensor([0])
                ...
                cams = mindspore.Tensor([result['cam'] for result in results[frame_id]])
                # to elimate the y-axis offset
                cams[:,2] -= 0.26
                trans = mindspore.Tensor([convert_cam_to_3d_trans(cam) for cam in cams])
                poses = mindspore.Tensor([result['poses'] for result in results[frame_id]])
                betas = mindspore.Tensor([result['betas'] for result in results[frame_id]])
                kp3ds = mindspore.Tensor([result['j3d_smpl24'] for result in results[frame_id]])
                verts = mindspore.Tensor([result['verts'] for result in results[frame_id]])
    ...
    def webcam_run_remote(self):
        ...
        while True:
            ...
            if outputs is not None:
                ...
                verts = verts * 50 + mindspore.Tensor([0, 0, 100])
                capture.send(verts)
            else:
                capture.send(['failed'])
