...
class Demo(Base):
    ...
    def net_forward(self,meta_data,mode='val'):
        ...
        meta_data['batch_ids'] = ms_np.arange(len(meta_data['image']))
    ...
    def test_cmu_panoptic(self):
        ...
        J_regressor_h36m = mindspore.Tensor.from_numpy(np.load(args().smpl_J_reg_h37m_path)).float()
        ...
        with ops.stop_gradient():
            for test_iter,meta_data in enumerate(data_loader):
                ...
                mpjpes = (ops.sqrt(((pred_kp3ds - gt_kp3ds) ** 2).sum(axis=-1)) * visible_kpts) *1000
                ...
                if test_iter%50==0:
                    ...
                    print('dataset bias: ', mindspore.Tensor(bias).mean(0))
        ...
        print('MPJPE results:', mindspore.Tensor(avg_all).mean())
...
def _calc_pn_fps(runtime_dict, person_num_crowd_index_dict):
    ...
    for person_num, runtime_list in person_num_runtime_dict.items():
        pn_fps[person_num] = 1./mindspore.Tensor(runtime_list).mean(0)
