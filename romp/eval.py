...
def calc_outputs_evaluation_matrix(self, outputs, ED):
    for ds in set(outputs['meta_data']['data_set']):
        val_idx = np.where(mindspore.Tensor(outputs['meta_data']['data_set']) == ds)[0]
        ...
        mPCKh = _calc_matched_PCKh_(outputs['meta_data']['full_kp2d'].float(), outputs['pj2d'].float(),
                                    outputs['meta_data']['valid_masks'][:, 0])
        ...
        if ds in constants.dataset_depth:
            ...
            relative_depth_errors = _calc_relative_depth_error_withgts_(trans_preds[:, 2], trans_gts[:, 2],
        ...
        if ds in ED['depth_relative']:
            age_gts = outputs['meta_data']['depth_info'][:, 0] if 'depth_info' in outputs['meta_data'] else None
            relative_depth_errors = _calc_relative_depth_error_weak_(outputs['cam_trans'][:, 2],
                                                                     outputs['meta_data']['depth_info'][:, 3],
                                                                     outputs['reorganize_idx'], age_gts=age_gts,
                                                                     matched_mask=matched_mask)
            for dr_type in constants.relative_depth_types:
                ED['depth_relative'][ds][dr_type] += relative_depth_errors[dr_type]
                ED['depth_relative'][ds][dr_type + '_age'] += relative_depth_errors[dr_type + '_age']
        if ds in ED['age_relative'] and args().learn_relative:
            relative_age_errors = _calc_relative_age_error_weak_(outputs['Age_preds'],
                                                                 outputs['meta_data']['depth_info'][:, 0],
                                                                 matched_mask=matched_mask)
        ...
        if ds not in constants.dataset_nokp3ds:
            if args().calc_PVE_error and ds in constants.PVE_ds:
                batch_PVE = ops.norm(
                    outputs['meta_data']['verts'][val_idx] - outputs['verts'][val_idx], p=2, axis=-1).mean(-1)
            ...
            if self.calc_pck:
                ...
                ED['PCK3D'][ds].append((mpjpe_pck_batch.reshape(-1) < self.PCK_thresh).astype(mindspore.float32) * 100)
                if ds in constants.MPJAE_ds:
                    rel_pose_pred = ops.cat(
                        [outputs['params']['global_orient'][val_idx], outputs['params']['body_pose'][val_idx]], 1)[:,
                                    :22 * 3].contiguous()
            ...
            ED['MPJPE'][ds].append(abs_error.astype(mindspore.float32))
            ED['PA_MPJPE'][ds].append(rt_error.astype(mindspore.float32))
            ED['imgpaths'][ds].append(mindspore.Tensor(outputs['meta_data']['imgpath'])[val_idx])
        else:
            kp3d_vis = None
    return ED, kp3d_vis
...
def val_result(self, loader_val, evaluation=False, vis_results=False):
    ...
    for iter_num, meta_data in enumerate(loader_val.create_dict_iterator()):
        ...
        if iter_num % (self.val_batch_size * 2) == 0:
            ...
            self.visualizer.visulize_result(outputs, outputs['meta_data'], show_items=show_items,
                                            vis_cfg={'settings': ['save_img'], 'vids': vis_ids,
                                                     'save_dir': self.result_img_dir, 'save_name': save_name},
                                            kp3ds=kp3d_vis)  # 'org_img',
...
def print_results(ED):
    ...
    for ds, results in ED['depth_relative'].items():
        ...
        if result_length > 0:
            eq_dists = ops.cat(ED['depth_relative'][ds]['eq'], 0)
            cd_dists = ops.cat(ED['depth_relative'][ds]['cd'], 0)
            fd_dists = ops.cat(ED['depth_relative'][ds]['fd'], 0)
            age_flag = len(ED['depth_relative'][ds]['eq_age']) > 0
            if age_flag:
                eq_age_ids = ops.cat(ED['depth_relative'][ds]['eq_age'], 0)
                cd_age_ids = ops.cat(ED['depth_relative'][ds]['cd_age'], 0)
                fd_age_ids = ops.cat(ED['depth_relative'][ds]['fd_age'], 0)
                dr_age_ids = ops.cat([eq_age_ids, cd_age_ids, fd_age_ids], 0)
            dr_all = mindspore.Tensor([len(eq_dists), len(cd_dists), len(fd_dists)])
            for dr_thresh in [0.2]:  # [0.1,0.15,0.2,0.25,0.3]:
                dr_corrects = [ops.abs(eq_dists) < dr_thresh, cd_dists < -dr_thresh, fd_dists > dr_thresh]
                print('Thresh: {} | Equal {} close {} far {}'.format(dr_thresh, dr_corrects[0].sum().item() / dr_all[0], \
                                                                     dr_corrects[1].sum().item() / dr_all[1],
                                                                     dr_corrects[2].sum().item() / dr_all[2]))
                dr_corrects = ops.cat(dr_corrects, 0)
                ...
                if age_flag:
                    for age_ind, age_name in enumerate(constants.relative_age_types):
                        ...
                        if age_mask.sum() > 0:
                            eval_results['{}-PCRD_{}_{}'.format(ds, dr_thresh, age_name)] = dr_corrects[
                                                                                                age_mask].sum() / age_mask.sum()
    ...
    for ds, results in ED['age_relative'].items():
        ...
        if result_length > 0:
            ...
            for age_id, age_type in enumerate(constants.relative_age_types):
                age_pred_ids = ops.cat(ED['age_relative'][ds][age_type], 0)
                ...
                eval_results['{}-acc_{}'.format(ds, age_type)] = age_error_results[age_type].sum() / len(
                    age_error_results[age_type])

            age_all_results = ops.cat(list(age_error_results.values()), 0)
    ...
    if args().calc_PVE_error:
        for ds_name in constants.PVE_ds:
            if len(ED['PVE_new'][ds_name]) > 0:
                eval_results['{}-PVE'.format(ds_name)] = ops.cat(ED['PVE_new'][ds_name], 0).mean() * 1000
    ...
    if len(ED['MPJPE']['h36m']) > 0:
        ...
        PA_MPJPE_acts = h36m_evaluation_act_wise(np.concatenate(ED['PA_MPJPE']['h36m'], axis=0),
                                                 np.concatenate(mindspore.Tensor(ED['imgpaths']['h36m']), axis=0),
                                                 constants.h36m_action_names)
        MPJPE_acts = h36m_evaluation_act_wise(np.concatenate(ED['MPJPE']['h36m'], axis=0),
                                              np.concatenate(mindspore.Tensor(ED['imgpaths']['h36m']), axis=0),
                                              constants.h36m_action_names)
...
def _init_error_dict():
    ED = {}
    ED['MPJPE'], ED['PA_MPJPE'], ED['PCK3D'], ED['imgpaths'] = [{ds: [] for ds in constants.dataset_involved} for _ in
                                                                range(4)]
    ...
    ED['depth_relative'] = {ds: {'eq': [], 'cd': [], 'fd': [], 'eq_age': [], 'cd_age': [], 'fd_age': []} for ds in
                            constants.dataset_relative_depth + constants.dataset_depth}
    ED['age_relative'] = {ds: {age_name: [] for age_name in constants.relative_age_types} for ds in
                          constants.dataset_relative_age}
    return ED
